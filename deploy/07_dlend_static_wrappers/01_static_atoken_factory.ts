...
import { DLEND_STATIC_A_TOKEN_FACTORY_ID, POOL_PROXY_ID } from "../../typescript/deploy-ids";
...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const staticATokenFactoryDeployment = await deployments.deploy(DLEND_STATIC_A_TOKEN_FACTORY_ID, {
    from: deployer,
  ...
  if (staticATokenFactoryDeployment.newlyDeployed) {
    ...
    const staticATokenFactory = await ethers.getContractAt("StaticATokenFactory", staticATokenFactoryDeployment.address);
    const pool = await ethers.getContractAt("contracts/dlend/core/interfaces/IPool.sol:IPool", poolAddress);
    ...
    if (reservesList.length > 0) {
      ...
      for (let chunkIndex = 0; chunkIndex < chunkedReserves.length; chunkIndex++) {
        ...
        try {
          ...
          const callData = (staticATokenFactory.interface as any).encodeFunctionData("createStaticATokens", [reservesChunk]);
          ...
          console.error(`  Failed to create StaticATokens for chunk ${chunkIndex + 1}: ${error.message || String(error)}`);
          // Re-throw to ensure the deployment script fails rather than silently succeeding.
          throw error;
        }
      }
    } else {
      console.log("No reserves found in the Pool, skipping createStaticATokens");
    }
  }
