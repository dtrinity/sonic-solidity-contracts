...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  if (!dSConfig?.initialFeeReceiver || !isAddress(dSConfig.initialFeeReceiver)) {
  ...
  const dSCollateralVaultDeployment = await get(DS_COLLATERAL_VAULT_CONTRACT_ID);
  const sOracleAggregator = await get(S_ORACLE_AGGREGATOR_ID);

  const dSRedeemerWithFeesDeployment = await deploy(DS_REDEEMER_WITH_FEES_CONTRACT_ID, {
    from: deployer,
    contract: "RedeemerWithFees",
    args: [
  ...
  if (!dSHasRole) {
    if (dSDeployerIsAdmin) {
      await dSCollateralVaultContract.grantRole(dSWithdrawerRole, dSRedeemerWithFeesDeployment.address);
