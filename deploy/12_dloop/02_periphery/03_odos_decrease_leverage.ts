...
import { DLOOP_PERIPHERY_ODOS_DECREASE_LEVERAGE_ID, DLOOP_PERIPHERY_ODOS_SWAP_LOGIC_ID } from "../../../typescript/deploy-ids";
...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  if (!dloopConfig || !dloopConfig.decreaseLeverage?.odos) {
    console.log(`No Odos decrease leverage configuration defined for network ${hre.network.name}. Skipping.`);
    return;
  }
  ...
  console.log(`Deploying Odos decrease leverage on network ${hre.network.name} (chainId: ${chainId})`);
...
func.tags = ["dloop", "dloop-periphery", "dloop-periphery-decrease-leverage", "dloop-periphery-decrease-leverage-odos"];
