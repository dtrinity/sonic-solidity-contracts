...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const ORACLE_MANAGER_ROLE = await ethers
    .getContractAt("OracleAggregator", governanceMultisig)
    .then((c) => c.ORACLE_MANAGER_ROLE())
    .catch(() => ""); // Assuming one of the wrappers has this role or getting it from OracleAggregator as a fallback

  if (!ORACLE_MANAGER_ROLE) {
    console.error(
      "❌ Could not determine ORACLE_MANAGER_ROLE. Skipping transfer for this role.",
    );
  }
  ...
  try {
    ...
    if (contractDeployment) {
      ...
      const contract = await ethers.getContractAt(
        "AccessControlEnumerable", // Assuming contracts use OpenZeppelin AccessControl
      ...
      if (!(await contract.hasRole(role, governanceMultisig))) {
        ...
        console.log(
          `    ✓ ${roleName} already granted to ${governanceMultisig}`,
        );
      }
    ...
    console.error(
      `  ❌ Failed to transfer ${contractName} ${roleName} roles: ${error}`,
    );
  }
