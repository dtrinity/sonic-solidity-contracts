...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment): Promise<boolean> {
  ...
  const api3WrapperDeployment = await hre.deployments.deploy(S_API3_ORACLE_WRAPPER_ID, {
    from: deployer,
  ...
  const api3WrapperWithThresholdingDeployment = await hre.deployments.deploy(S_API3_WRAPPER_WITH_THRESHOLDING_ID, {
    from: deployer,
    args: [baseCurrency, baseCurrencyUnit],
  ...
  const api3CompositeWrapperDeployment = await hre.deployments.deploy(S_API3_COMPOSITE_WRAPPER_WITH_THRESHOLDING_ID, {
    from: deployer,
    args: [baseCurrency, baseCurrencyUnit],
...
func.tags = ["s-oracle", "oracle-aggregator", "oracle-wrapper", "s-api3-oracle-wrapper"];
func.dependencies = [];
