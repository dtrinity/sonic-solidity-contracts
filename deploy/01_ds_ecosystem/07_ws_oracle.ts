...
import { S_ORACLE_AGGREGATOR_ID, WS_HARD_PEG_ORACLE_WRAPPER_ID } from "../../typescript/deploy-ids";
...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const { address: hardPegOracleWrapperAddress } = await hre.deployments.get(WS_HARD_PEG_ORACLE_WRAPPER_ID);

  // Set the HardPegOracleWrapper as the oracle for wS
  console.log(`Setting HardPegOracleWrapper for wS (${config.tokenAddresses.wS}) to`, hardPegOracleWrapperAddress);
  await oracleAggregatorContract.setOracle(config.tokenAddresses.wS, hardPegOracleWrapperAddress);
