...
import { DS_HARD_PEG_ORACLE_WRAPPER_ID, S_ORACLE_AGGREGATOR_ID } from "../../typescript/deploy-ids";
...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const { address: hardPegOracleWrapperAddress } = await hre.deployments.get(DS_HARD_PEG_ORACLE_WRAPPER_ID);

  // Set the HardPegOracleWrapper as the oracle for dUSD
  console.log(`Setting HardPegOracleWrapper for dS (${config.tokenAddresses.dS}) to`, hardPegOracleWrapperAddress);
  await oracleAggregatorContract.setOracle(config.tokenAddresses.dS, hardPegOracleWrapperAddress);
