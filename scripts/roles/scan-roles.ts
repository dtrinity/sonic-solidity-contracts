...
async function main() {
  ...
  for (const c of result.rolesContracts) {
    ...
    if (c.rolesHeldByDeployer.length > 0) {
      console.log(
        `  deployer roles: ${c.rolesHeldByDeployer.map((r) => r.name).join(", ")}`
      );
    }
    if (c.rolesHeldByGovernance.length > 0) {
      console.log(
        `  governance roles: ${c.rolesHeldByGovernance.map((r) => r.name).join(", ")}`
      );
    }
  ...
  for (const c of result.ownableContracts) {
    console.log(
      `- ${c.name} (${c.address}) owner=${c.owner} deployerIsOwner=${c.deployerIsOwner}`
    );
  }

  // Final exposure summary
  const exposureRoles = result.rolesContracts.filter(
    (c) => c.rolesHeldByDeployer.length > 0
  );
  const exposureOwnable = result.ownableContracts.filter(
    (c) => c.deployerIsOwner
  );
  ...
  if (exposureRoles.length > 0) {
    console.log(
      `Contracts with roles held by deployer: ${exposureRoles.length}`
    );
