...
async function main() {
  ...
  for (const c of scan.rolesContracts) {
    ...
    for (const role of nonAdminRolesHeldByDeployer) {
      console.log(`  - Grant ${role.name} to governance, then renounce from deployer`);
      addedOperations += 2;
  ...
  for (const c of scan.rolesContracts) {
    ...
    for (const role of nonAdminRolesHeldByDeployer) {
      try {
        const governanceHasRole: boolean = await contract.hasRole(role.hash, governance);
        if (!governanceHasRole) {
          console.log(`Granting ${role.name} to governance on ${c.name} (${c.address})...`);
          const tx = await contract.grantRole(role.hash, governance);
          await tx.wait();
        } else {
          console.log(`Governance already has ${role.name} on ${c.name}; skipping grant.`);
        }

        const deployerHasRole: boolean = await contract.hasRole(role.hash, deployer);
        if (deployerHasRole) {
          console.log(`Renouncing ${role.name} from deployer on ${c.name} (${c.address})...`);
          const tx2 = await contract.renounceRole(role.hash, deployer);
          await tx2.wait();
        } else {
          console.log(`Deployer does not hold ${role.name} on ${c.name}; skipping renounce.`);
        }
      } catch (e) {
        console.error(`Error migrating role ${role.name} on ${c.name} (${c.address}):`, e);
      }
    }
    ...
    if (adminRole) {
      try {
        const governanceHasAdmin: boolean = await contract.hasRole(adminRole.hash, governance);
        if (!governanceHasAdmin) {
          console.log(`Granting DEFAULT_ADMIN_ROLE to governance on ${c.name} (${c.address})...`);
          const tx = await contract.grantRole(adminRole.hash, governance);
          await tx.wait();
        } else {
          console.log(`Governance already has DEFAULT_ADMIN_ROLE on ${c.name}; skipping grant.`);
        }

        const deployerHasAdmin: boolean = await contract.hasRole(adminRole.hash, deployer);
        if (deployerHasAdmin) {
          console.log(`Renouncing DEFAULT_ADMIN_ROLE from deployer on ${c.name} (${c.address})...`);
          const tx2 = await contract.renounceRole(adminRole.hash, deployer);
          await tx2.wait();
        } else {
          console.log(`Deployer does not hold DEFAULT_ADMIN_ROLE on ${c.name}; skipping renounce.`);
        }
      } catch (e) {
        console.error(`Error migrating DEFAULT_ADMIN_ROLE on ${c.name} (${c.address}):`, e);
      }
    }
  }
  ...
  for (const c of scan.ownableContracts) {
    ...
    try {
      ...
      if (currentOwner.toLowerCase() === governance.toLowerCase()) {
        console.log(`Governance already owns ${c.name} (${c.address}); skipping.`);
        continue;
      }
      console.log(`Transferring ownership of ${c.name} (${c.address}) to governance...`);
      const tx = await contract.transferOwnership(governance);
      await tx.wait();
    } catch (e) {
      console.error(`Error transferring ownership for ${c.name} (${c.address}):`, e);
    }
  }
}
