  ...
  describe.skip(`DEPRECATED: Redeemer for ${config.symbol}`, () => {
    ...
    beforeEach(async function () {
      ...
      for (const collateralSymbol of config.peggedCollaterals) {
        ...
        const expectedDstableAmount = await issuerContract.baseValueToDstableAmount(
          await collateralVaultContract.assetValueFromAmount(userAmount, result.tokenInfo.address),
        );
    ...
    describe("Basic redemption", () => {
      ...
      config.peggedCollaterals.forEach((collateralSymbol) => {
        it(`redeems ${config.symbol} for ${collateralSymbol}`, async function () {
          ...
          const expectedCollateralAmount = await calculateExpectedCollateralAmount(
            redeemAmount,
            dstableInfo.symbol,
          ...
          const minCollateralOut = (expectedCollateralAmount * BigInt(100 - slippagePercentage)) / 100n;
          ...
          const receivedCollateral = userCollateralBalanceAfter - userCollateralBalanceBefore;
          ...
          assert.isTrue(
            receivedCollateral >= lowerBound && receivedCollateral <= upperBound,
        ...
        it(`cannot redeem ${config.symbol} when slippage is too high`, async function () {
          ...
          await expect(
            redeemerContract
              ...
              .redeem(redeemAmount, collateralInfo.address, unrealistically_high_min_collateral),
    ...
    describe("Administrative functions", () => {
      it("allows changing the collateral vault", async function () {
        ...
        assert.equal(updatedVault, newVault.address, "Collateral vault should be updated");
      });
      ...
      it("allows admin to grant REDEMPTION_MANAGER_ROLE", async function () {
        ...
        const hasRole = await redeemerContract.hasRole(REDEMPTION_MANAGER_ROLE, user2);
