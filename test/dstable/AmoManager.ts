...
import { createDStableAmoFixture, DS_CONFIG, DStableFixtureConfig, DUSD_CONFIG } from "./fixtures";
...
async function runTestsForDStable(
  ...
  describe(`AmoManager for ${config.symbol}`, () => {
    ...
    beforeEach(async function () {
      ...
      mockAmoVault = await hre.ethers.getContractAt("MockAmoVault", mockAmoVaultAddress, await hre.ethers.getSigner(deployer));
      ...
      const firstCollateralAddress = collateralAddresses.find((addr) => addr !== hre.ethers.ZeroAddress);
      ...
      const { contract, tokenInfo } = await getTokenContractForAddress(hre, deployer, firstCollateralAddress);
      ...
      if (!(await mockAmoVault.hasRole(collateralWithdrawerRole, amoManagerAddress))) {
        await mockAmoVault.grantRole(collateralWithdrawerRole, amoManagerAddress);
      }

      // Grant necessary roles to AmoManager for collateral management
      if (!(await collateralVaultContract.hasRole(collateralWithdrawerRole, await amoManagerContract.getAddress()))) {
        await collateralVaultContract.grantRole(collateralWithdrawerRole, await amoManagerContract.getAddress());
      }
    ...
    describe("Role-based access control", () => {
      it("should have correct role assignments after deployment", async function () {
        expect(await amoManagerContract.hasRole(defaultAdminRole, deployer)).to.be.true;
        expect(await amoManagerContract.hasRole(amoAllocatorRole, deployer)).to.be.true;
        expect(await amoManagerContract.hasRole(feeCollectorRole, deployer)).to.be.true;
      });
      ...
      it("should prevent unauthorized users from accessing restricted functions", async function () {
        ...
        await expect(
          amoManagerContract.connect(unauthorizedSigner).allocateAmo(await mockAmoVault.getAddress(), amount),
        ).to.be.revertedWithCustomError(amoManagerContract, "AccessControlUnauthorizedAccount");
      });
    });
    ...
    describe("AMO allocation", () => {
      it("allocates AMO tokens to an active vault", async function () {
        const amoVault = await mockAmoVault.getAddress();
        const allocateAmount = hre.ethers.parseUnits("1000", dstableInfo.decimals);
        ...
        assert.equal(finalAmoSupply.toString(), initialAmoSupply.toString(), "Total AMO supply should not change");
        assert.equal(finalVaultBalance - initialVaultBalance, allocateAmount, "Vault balance should increase by allocated amount");
      });
      ...
      it("cannot allocate to an inactive vault", async function () {
        ...
        await expect(amoManagerContract.allocateAmo(inactiveVault, allocateAmount)).to.be.revertedWithCustomError(
          amoManagerContract,
          "InactiveAmoVault",
        );
      });
      ...
      it("cannot allocate more than unallocated supply", async function () {
        ...
        await expect(amoManagerContract.allocateAmo(amoVault, allocateAmount)).to.be.reverted;
      });
    });
    ...
    describe("AMO deallocation", () => {
      ...
      it("deallocates AMO tokens from an active vault", async function () {
        ...
        await hre.network.provider.send("hardhat_setBalance", [amoVault, "0x1000000000000000000"]);
        ...
        await dstableContract.connect(amoVaultSigner).approve(await amoManagerContract.getAddress(), deallocateAmount);

        const initialVaultBalance = await dstableContract.balanceOf(amoVault);
        const initialAmoManagerBalance = await dstableContract.balanceOf(await amoManagerContract.getAddress());

        await amoManagerContract.deallocateAmo(amoVault, deallocateAmount);

        const finalVaultBalance = await dstableContract.balanceOf(amoVault);
        const finalAmoManagerBalance = await dstableContract.balanceOf(await amoManagerContract.getAddress());
        ...
        assert.equal(initialVaultBalance - finalVaultBalance, deallocateAmount, "Vault balance should decrease by deallocated amount");
        assert.equal(
      ...
      it("cannot deallocate more than allocated to vault", async function () {
        ...
        await expect(amoManagerContract.deallocateAmo(amoVault, deallocateAmount)).to.be.revertedWithCustomError(
          amoManagerContract,
    ...
    describe("AMO vault management", () => {
      it("enables an AMO vault", async function () {
        ...
        await expect(amoManagerContract.allocateAmo(vault, 1n)).to.be.revertedWithCustomError(amoManagerContract, "InactiveAmoVault");
        ...
        assert.equal(vaultBalance, allocateAmount, "Vault should receive allocated tokens after enabling");
      });
      ...
      it("disables an AMO vault", async function () {
        ...
        await expect(amoManagerContract.allocateAmo(vault, allocateAmount)).to.be.revertedWithCustomError(
          amoManagerContract,
          "InactiveAmoVault",
        );
      });
      ...
      it("cannot enable an already enabled vault", async function () {
        ...
        await expect(amoManagerContract.enableAmoVault(vault)).to.be.revertedWithCustomError(amoManagerContract, "AmoVaultAlreadyEnabled");
      });
      ...
      it("cannot disable an already disabled vault", async function () {
        ...
        await expect(amoManagerContract.disableAmoVault(vault)).to.be.revertedWithCustomError(amoManagerContract, "InactiveAmoVault");
      });
    });
    ...
    describe("AMO supply management", () => {
      it("decreases AMO supply by burning dStable", async function () {
        ...
        assert.equal(initialAmoSupply - finalAmoSupply, burnAmount, "AMO supply should decrease by burn amount");
        assert.equal(initialTotalSupply - finalTotalSupply, burnAmount, "dStable total supply should decrease by burn amount");
      });
      ...
      it("cannot decrease AMO supply more than available", async function () {
        ...
        await expect(amoManagerContract.decreaseAmoSupply(burnAmount)).to.be.reverted;
      });
    });
    ...
    describe("Collateral Management", () => {
      ...
      it("transfers collateral from AMO vault to holding vault", async function () {
        const amoVault = await mockAmoVault.getAddress();
        const collateralAmount = hre.ethers.parseUnits("100", testCollateralInfo.decimals);
        ...
        const initialHoldingVaultBalance = await testCollateralToken.balanceOf(await collateralVaultContract.getAddress());

        await amoManagerContract.transferFromAmoVaultToHoldingVault(amoVault, await testCollateralToken.getAddress(), collateralAmount);

        const finalHoldingVaultBalance = await testCollateralToken.balanceOf(await collateralVaultContract.getAddress());

        expect(finalHoldingVaultBalance - initialHoldingVaultBalance).to.equal(collateralAmount);
      });
      ...
      it("transfers collateral from holding vault to AMO vault", async function () {
        ...
        await testCollateralToken.transfer(await collateralVaultContract.getAddress(), collateralAmount);

        const initialAmoVaultBalance = await testCollateralToken.balanceOf(amoVault);

        await amoManagerContract.transferFromHoldingVaultToAmoVault(amoVault, await testCollateralToken.getAddress(), collateralAmount);

        const finalAmoVaultBalance = await testCollateralToken.balanceOf(amoVault);

        expect(finalAmoVaultBalance - initialAmoVaultBalance).to.equal(collateralAmount);
      });
      ...
      it("cannot transfer dStable as collateral", async function () {
        ...
        await expect(
          amoManagerContract.transferFromAmoVaultToHoldingVault(amoVault, await dstableContract.getAddress(), amount),
        ).to.be.revertedWithCustomError(amoManagerContract, "CannotTransferDStable");

        await expect(
          amoManagerContract.transferFromHoldingVaultToAmoVault(amoVault, await dstableContract.getAddress(), amount),
        ).to.be.revertedWithCustomError(amoManagerContract, "CannotTransferDStable");
      });
      ...
      it("does not reactivate a disabled vault when transferring collateral", async function () {
        ...
        const collateralAmount = hre.ethers.parseUnits("50", testCollateralInfo.decimals);
        ...
        const initialHoldingBalance = await testCollateralToken.balanceOf(await collateralVaultContract.getAddress());
        ...
        const finalHoldingBalance = await testCollateralToken.balanceOf(await collateralVaultContract.getAddress());
        expect(finalHoldingBalance - initialHoldingBalance).to.equal(collateralAmount);

        // Verify that attempting to allocate to this vault now reverts
        await expect(
          amoManagerContract.allocateAmo(amoVault, hre.ethers.parseUnits("10", dstableInfo.decimals)),
      ...
      it("emits AllocationSurplus when collateral value exceeds allocation", async function () {
        ...
        const collateralAmount = hre.ethers.parseUnits("25", testCollateralInfo.decimals);

        // Transfer collateral to the AMO vault from deployer (simulate profit)
        await testCollateralToken.transfer(amoVault, collateralAmount);

        // Compute expected surplus in dStable equivalent
        const collateralBaseValue = await collateralVaultContract.assetValueFromAmount(
          collateralAmount,
          await testCollateralToken.getAddress(),
        );
        const collateralInDstable = await amoManagerContract.baseValueToDstableAmount(collateralBaseValue);

        await expect(
          amoManagerContract.transferFromAmoVaultToHoldingVault(amoVault, await testCollateralToken.getAddress(), collateralAmount),
        )
    ...
    describe("Profit Management", () => {
      ...
      it("calculates available vault profits correctly", async function () {
        ...
        const profitAmount = hre.ethers.parseUnits("100", testCollateralInfo.decimals);
        await testCollateralToken.transfer(amoVault, profitAmount);

        const availableProfit = await amoManagerContract.availableVaultProfitsInBase(amoVault);
      ...
      it("withdraws profits from AMO vault", async function () {
        ...
        const initialRecipientBalance = await testCollateralToken.balanceOf(recipient);

        // Try to withdraw profits
        await amoManagerContract.withdrawProfits(mockAmoVault, recipient, await testCollateralToken.getAddress(), profitAmount);

        const finalRecipientBalance = await testCollateralToken.balanceOf(recipient);

        expect(finalRecipientBalance - initialRecipientBalance).to.equal(profitAmount);
      });
      ...
      it("cannot withdraw more than available profits", async function () {
        ...
        const availableAmount = hre.ethers.parseUnits("100", testCollateralInfo.decimals);
        ...
        await expect(
          amoManagerContract.withdrawProfits(mockAmoVault, recipient, await testCollateralToken.getAddress(), withdrawAmount),
    ...
    describe("Base value conversion", () => {
      it("converts base value to dStable amount correctly", async function () {
        ...
        const dstableAmount = await amoManagerContract.baseValueToDstableAmount(baseValue);

        // Convert back to base value
        const convertedBaseValue = await amoManagerContract.dstableAmountToBaseValue(dstableAmount);

        // Allow for small rounding errors
        const difference = convertedBaseValue > baseValue ? convertedBaseValue - baseValue : baseValue - convertedBaseValue;
        const acceptableError = (baseValue * 1n) / 100n; // 1% error margin

        expect(difference).to.be.lte(acceptableError, "Base value conversion should be within acceptable error margin");
      });
      ...
      it("converts dStable amount to base value correctly", async function () {
        ...
        const baseValue = await amoManagerContract.dstableAmountToBaseValue(dstableAmount);

        // Convert back to dStable amount
        const convertedDstableAmount = await amoManagerContract.baseValueToDstableAmount(baseValue);

        // Allow for small rounding errors
        const difference =
          convertedDstableAmount > dstableAmount ? convertedDstableAmount - dstableAmount : dstableAmount - convertedDstableAmount;
        const acceptableError = (dstableAmount * 1n) / 100n; // 1% error margin

        expect(difference).to.be.lte(acceptableError, "dStable amount conversion should be within acceptable error margin");
      });
    });
    ...
    describe("Admin functions", () => {
      it("allows admin to set collateral vault", async function () {
        ...
        expect(await amoManagerContract.collateralHolderVault()).to.equal(newCollateralVault);
      });
      ...
      it("prevents non-admin from setting collateral vault", async function () {
        ...
        await expect(amoManagerContract.connect(nonAdmin).setCollateralVault(newCollateralVault)).to.be.revertedWithCustomError(
          amoManagerContract,
    ...
    describe("AmoVault manager allowance", () => {
      it("revokes the old manager allowance and grants it to the new one", async function () {
        ...
        expect(await dstableContract.allowance(amoVaultAddress, oldManagerAddress)).to.equal(maxUint);
        ...
        expect(await dstableContract.allowance(amoVaultAddress, oldManagerAddress)).to.equal(0n);

        // New manager should have max allowance
        expect(await dstableContract.allowance(amoVaultAddress, newManagerAddress)).to.equal(maxUint);
      });
      ...
      it("allows admin to set a custom allowance for the current manager", async function () {
        ...
        const customAllowance = hre.ethers.parseUnits("5000", dstableInfo.decimals);
        ...
        expect(await dstableContract.allowance(amoVaultAddress, managerAddress)).to.equal(customAllowance);
      });
    });
  });
}
