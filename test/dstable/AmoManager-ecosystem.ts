...
dstableConfigs.forEach((config) => {
  describe(`AmoManager Ecosystem Tests for ${config.symbol}`, () => {
    ...
    beforeEach(async function () {
      ...
      expect(await mockAmoVaultContract.amoManager()).to.equal(amoManagerAddress);
      expect(await mockAmoVaultContract.oracle()).to.equal(oracleAggregatorAddress);

      // Initialize all collateral tokens for this dStable
      for (const collateralSymbol of [...config.peggedCollaterals, ...config.yieldBearingCollaterals]) {
    ...
    describe("AMO ecosystem interactions", () => {
      it("verifies oracle prices for pegged and yield-bearing collateral", async function () {
        ...
        for (const symbol of config.peggedCollaterals) {
          ...
          const price = await oracleAggregatorContract.getAssetPrice(collateralInfo.address);
          const expectedPrice = hre.ethers.parseUnits("1", 18); // API3 uses 18 decimals
          assert.equal(price, expectedPrice, `Pegged collateral ${symbol} should have 1:1 price ratio`);
        }
        ...
        for (const symbol of config.yieldBearingCollaterals) {
          ...
          assert.equal(price, expectedPrice, `Yield-bearing collateral ${symbol} should have 1:1.1 price ratio`);
        }
      });
      ...
      it("calculates profit correctly with yield-bearing collateral", async function () {
        ...
        const dstableToAllocate = hre.ethers.parseUnits("1000", dstableInfo.decimals);
        ...
        await collateralContract.approve(await mockAmoVaultContract.getAddress(), collateralAmount);
      ...
      it("calculates vault value with various assets", async function () {
        ...
        const peggedCollateralContract = collateralContracts.get(peggedCollateralSymbol) as TestERC20;
        const peggedCollateralInfo = collateralInfos.get(peggedCollateralSymbol) as TokenInfo;

        const peggedCollateralAmount = hre.ethers.parseUnits("500", peggedCollateralInfo.decimals);

        // Approve and deposit pegged collateral
        await peggedCollateralContract.approve(await mockAmoVaultContract.getAddress(), peggedCollateralAmount);
        await mockAmoVaultContract.deposit(peggedCollateralAmount, peggedCollateralInfo.address);
        ...
        if (config.yieldBearingCollaterals.length > 0) {
          const yieldBearingCollateralSymbol = config.yieldBearingCollaterals[0];
          const yieldBearingCollateralContract = collateralContracts.get(yieldBearingCollateralSymbol) as TestERC20;
          yieldBearingCollateralInfo = collateralInfos.get(yieldBearingCollateralSymbol) as TokenInfo;
        ...
        const fakeDeFiValue = hre.ethers.parseUnits("200", ORACLE_AGGREGATOR_PRICE_DECIMALS);
        ...
        const collateralValue = await mockAmoVaultContract.totalCollateralValue();
        ...
        if (yieldBearingCollateralInfo && yieldBearingCollateralAmount > 0n) {
          expectedYieldBearingCollateralValue = await calculateBaseValueFromAmount(
      ...
      it("transfers collateral between AMO vault and collateral vault", async function () {
        ...
        const testTransfer = async (collateralSymbol: string) => {
          ...
          const collateralAmount = hre.ethers.parseUnits("500", collateralInfo.decimals);
          await collateralContract.transfer(await mockAmoVaultContract.getAddress(), collateralAmount);

          // Check initial balances
          const initialAmoVaultBalance = await collateralContract.balanceOf(await mockAmoVaultContract.getAddress());
          const initialVaultBalance = await collateralContract.balanceOf(await collateralVaultContract.getAddress());
          ...
          const finalAmoVaultBalance = await collateralContract.balanceOf(await mockAmoVaultContract.getAddress());
          const finalVaultBalance = await collateralContract.balanceOf(await collateralVaultContract.getAddress());
