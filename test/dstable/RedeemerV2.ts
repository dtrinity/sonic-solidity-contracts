...
async function calculateExpectedCollateralAmount(
  ...
  const dstableBaseValue = (dstableAmount * dstablePrice) / 10n ** BigInt(dstableDecimals);
  return (dstableBaseValue * 10n ** BigInt(collateralDecimals)) / collateralPrice;
}
...
dstableConfigs.forEach((config) => {
  describe(`RedeemerV2 for ${config.symbol}`, () => {
    ...
    beforeEach(async function () {
      ...
      // Deploy RedeemerV2
      const RedeemerV2Factory = await hre.ethers.getContractFactory("RedeemerV2", await hre.ethers.getSigner(deployer));
      ...
      await collateralVault.grantRole(await collateralVault.COLLATERAL_WITHDRAWER_ROLE(), await redeemer.getAddress());

      // Mint user1 some dStable by issuing through IssuerV2 with first collateral
      ...
      await firstToken.connect(await hre.ethers.getSigner(user1)).approve(await issuer.getAddress(), depositAmount);
      await issuer.connect(await hre.ethers.getSigner(user1)).issue(depositAmount, firstInfo.address, 0);
    ...
    describe("Deployment and Configuration", () => {
      it("has zero default fee and feeReceiver set to deployer by default", async function () {
        ...
        assert.equal(feeReceiver, deployer, "feeReceiver should default to deployer");
        assert.equal(defaultFee, 0n, "default fee bps should be zero by default");
      });
      ...
      it("revokes withdrawer role from legacy Redeemer and RedeemerWithFees", async function () {
        ...
        const legacyIds = [
          config.symbol === "dUSD" ? DUSD_REDEEMER_CONTRACT_ID : DS_REDEEMER_CONTRACT_ID,
          config.symbol === "dUSD" ? DUSD_REDEEMER_WITH_FEES_CONTRACT_ID : DS_REDEEMER_WITH_FEES_CONTRACT_ID,
        ];
        ...
        for (const id of legacyIds) {
          ...
          expect(has).to.equal(false, `${id} should not retain withdrawer role`);
        }
      });
    });
    ...
    it("per-asset pause prevents redemption", async function () {
      ...
      await dstable.connect(await hre.ethers.getSigner(user1)).approve(await redeemer.getAddress(), amount);

      await expect(redeemer.connect(await hre.ethers.getSigner(user1)).redeem(amount, info.address, 0))
    ...
    it("global pause prevents redemption and unpause restores", async function () {
      ...
      await expect(redeemer.connect(await hre.ethers.getSigner(user1)).redeem(amount, info.address, 0)).to.be.revertedWithCustomError(
        redeemer,
        "EnforcedPause",
      );
      ...
      await redeemer.connect(await hre.ethers.getSigner(user1)).redeem(amount, info.address, 0);
    });
    ...
    it("only PAUSER_ROLE can set asset pause/unpause", async function () {
      ...
      await expect(redeemer.connect(await hre.ethers.getSigner(user1)).setAssetRedemptionPause(info.address, true))
        .to.be.revertedWithCustomError(redeemer, "AccessControlUnauthorizedAccount")
    ...
    it("redemption computes amounts correctly within slippage bounds", async function () {
      ...
      const userBalBefore = await (await hre.ethers.getContractAt("TestERC20", info.address, await hre.ethers.getSigner(deployer)))
        .connect(await hre.ethers.getSigner(user1))
        .balanceOf(user1);

      await redeemer.connect(await hre.ethers.getSigner(user1)).redeem(amount, info.address, minOut);

      const userBalAfter = await (await hre.ethers.getContractAt("TestERC20", info.address, await hre.ethers.getSigner(deployer)))
        .connect(await hre.ethers.getSigner(user1))
        .balanceOf(user1);
    ...
    describe("Public Redemption with Fees", () => {
      it("applies default fee on public redemption and emits Redemption", async function () {
        ...
        await dstable.connect(userSigner).approve(await redeemer.getAddress(), redeemAmount);

        const dstableValue = await redeemer.dstableAmountToBaseValue(redeemAmount);
        const totalCollateral = await collateralVault.assetAmountFromValue(dstableValue, collateralInfo.address);
        const expectedFee = (totalCollateral * newFee) / BigInt(ONE_HUNDRED_PERCENT_BPS);
        ...
        const vaultBefore = await collateralToken.balanceOf(await collateralVault.getAddress());

        const tx = await redeemer.connect(userSigner).redeem(redeemAmount, collateralInfo.address, 0);

        await expect(tx).to.emit(redeemer, "Redemption").withArgs(user1, collateralInfo.address, redeemAmount, expectedNet, expectedFee);

        const userAfter = await collateralToken.balanceOf(user1);
        const feeAfter = await collateralToken.balanceOf(feeReceiver);
        const vaultAfter = await collateralToken.balanceOf(await collateralVault.getAddress());

        assert.equal(userAfter - userBefore, expectedNet, "User should receive net amount after fee");
        assert.equal(feeAfter - feeBefore, expectedFee, "Fee receiver should receive fee amount");
        assert.equal(vaultBefore - vaultAfter, totalCollateral, "Vault decreases by total collateral redeemed");
      });
    });
    ...
    describe("Protocol Redemption (No Fees)", () => {
      it("allows manager to redeem without fees", async function () {
        ...
        await dstable.connect(managerSigner).approve(await redeemer.getAddress(), redeemAmount);
        const before = await collateralToken.balanceOf(user1);
        await expect(redeemer.connect(managerSigner).redeemAsProtocol(redeemAmount, collateralInfo.address, 0)).to.emit(
          redeemer,
          "Redemption",
        );
        const after = await collateralToken.balanceOf(user1);
        assert.isTrue(after > before, "Manager should receive full collateral amount without fee");
      });
      ...
      it("reverts if non-manager calls redeemAsProtocol", async function () {
        ...
        await expect(redeemer.connect(other).redeemAsProtocol(redeemAmount, collateralInfo.address, 0)).to.be.reverted;
      });
    });
    ...
    describe("Administrative functions", () => {
      it("allows admin to set fee receiver", async function () {
        ...
        assert.equal(await redeemer.feeReceiver(), user1, "Fee receiver should be updated");
      });

      it("reverts when non-admin tries to set fee receiver", async function () {
        await expect(redeemer.connect(await hre.ethers.getSigner(user1)).setFeeReceiver(await redeemer.getAddress())).to.be.reverted;
      });
      ...
      it("allows admin to set default redemption fee and enforces max", async function () {
        ...
        assert.equal((await redeemer.defaultRedemptionFeeBps()).toString(), "200");

        const maxPlusOne = (await redeemer.MAX_FEE_BPS()) + 1n;
        await expect(redeemer.setDefaultRedemptionFee(maxPlusOne)).to.be.revertedWithCustomError(redeemer, "FeeTooHigh");
      });
      ...
      it("allows admin to set collateral-specific fee and enforces max", async function () {
        ...
        assert.equal((await redeemer.collateralRedemptionFeeBps(collateralInfo.address)).toString(), "300");
        const maxPlusOne = (await redeemer.MAX_FEE_BPS()) + 1n;
        await expect(redeemer.setCollateralRedemptionFee(collateralInfo.address, maxPlusOne)).to.be.revertedWithCustomError(
          redeemer,
          "FeeTooHigh",
        );
      });
      ...
      it("supports clearing per-asset override (0 bps override -> clear -> fallback to default)", async function () {
        ...
        assert.equal(await redeemer.isCollateralFeeOverridden(collateralInfo.address), true);
        ...
        const dstableValue0 = await redeemer.dstableAmountToBaseValue(redeemAmount);
        const totalCollateral0 = await collateralVault.assetAmountFromValue(dstableValue0, collateralInfo.address);
        ...
        await redeemer.connect(userSigner).redeem(redeemAmount, collateralInfo.address, 0);

        const userAfter0 = await collateralToken.balanceOf(user1);
        const feeAfter0 = await collateralToken.balanceOf(feeReceiver);

        assert.equal(userAfter0 - userBefore0, expectedNet0, "user should receive full collateral under 0 bps override");
        assert.equal(feeAfter0 - feeBefore0, expectedFee0, "fee receiver should not receive fee under 0 bps override");

        // Clear override -> should fallback to default
        await redeemer.clearCollateralRedemptionFee(collateralInfo.address);
        assert.equal(await redeemer.isCollateralFeeOverridden(collateralInfo.address), false);

        // Redeem again, expect default fee applied
        const redeemAmount2 = hre.ethers.parseUnits("20", dstableInfo.decimals);
        await dstable.connect(userSigner).approve(await redeemer.getAddress(), redeemAmount2);
        const dstableValue2 = await redeemer.dstableAmountToBaseValue(redeemAmount2);
        const totalCollateral2 = await collateralVault.assetAmountFromValue(dstableValue2, collateralInfo.address);
        const expectedFee2 = (totalCollateral2 * defaultFee) / BigInt(ONE_HUNDRED_PERCENT_BPS);
        ...
        await redeemer.connect(userSigner).redeem(redeemAmount2, collateralInfo.address, 0);

        const userAfter2 = await collateralToken.balanceOf(user1);
        const feeAfter2 = await collateralToken.balanceOf(feeReceiver);

        assert.equal(userAfter2 - userBefore2, expectedNet2, "user should receive net amount after default fee once override cleared");
        assert.equal(feeAfter2 - feeBefore2, expectedFee2, "fee receiver should receive default fee once override cleared");
      });
    });
  });
});
