  ...
  describe(`CollateralHolderVault for ${config.symbol}`, () => {
    ...
    beforeEach(async function () {
      ...
      for (const collateralSymbol of config.peggedCollaterals) {
        const { contract, tokenInfo } = await getTokenContractForSymbol(hre, deployer, collateralSymbol);
    ...
    describe("Collateral management", () => {
      ...
      config.peggedCollaterals.forEach((collateralSymbol) => {
        it(`allows ${collateralSymbol} as collateral`, async function () {
          ...
          const isSupported = await collateralVaultContract.isCollateralSupported(collateralInfo.address);
          assert.isTrue(isSupported, `${collateralSymbol} should be supported as collateral`);

          // There's no direct method to check if collateral is allowed, so we'll test by trying to deposit
          const depositAmount = hre.ethers.parseUnits("1", collateralInfo.decimals);
          const collateralContract = collateralContracts.get(collateralSymbol) as TestERC20;

          await collateralContract.approve(await collateralVaultContract.getAddress(), depositAmount);

          // If this doesn't revert, then collateral is allowed
          await collateralVaultContract.deposit(depositAmount, collateralInfo.address);
        });
        ...
        it(`allows depositing ${collateralSymbol}`, async function () {
          ...
          const depositAmount = hre.ethers.parseUnits("500", collateralInfo.decimals);
        ...
        it(`disallows depositing non-allowed ${collateralSymbol}`, async function () {
          ...
          const nonAllowedCollateral = "0x0000000000000000000000000000000000000123";
          ...
          await expect(collateralVaultContract.deposit(depositAmount, nonAllowedCollateral))
    ...
    describe("Base value calculations", () => {
      ...
      it("calculates total value correctly", async function () {
        ...
        for (const collateralSymbol of config.peggedCollaterals) {
          ...
          const depositAmount = hre.ethers.parseUnits("100", collateralInfo.decimals);
          ...
          const collateralValue = await calculateBaseValueFromAmount(depositAmount, collateralInfo.address);
      ...
      it("correctly converts between base value and asset amount", async function () {
        ...
        const assetAmount = await collateralVaultContract.assetAmountFromValue(baseValue, collateralInfo.address);

        // Calculate the expected asset amount using oracle prices
        const expectedAssetAmount = await calculateAmountFromBaseValue(baseValue, collateralInfo.address);
    ...
    describe("Administrative functions", () => {
      it("allows authorized withdrawals", async function () {
        ...
        await collateralVaultContract.grantRole(COLLATERAL_WITHDRAWER_ROLE, user2);
    ...
    if (config.peggedCollaterals.length >= 2) {
      describe("exchangeCollateral", () => {
        ...
        beforeEach(async function () {
          fromInfo = collateralInfos.get(config.peggedCollaterals[fromIndex]) as TokenInfo;
          toInfo = collateralInfos.get(config.peggedCollaterals[toIndex]) as TokenInfo;

          fromContract = collateralContracts.get(config.peggedCollaterals[fromIndex]) as TestERC20;
          toContract = collateralContracts.get(config.peggedCollaterals[toIndex]) as TestERC20;

          // 1. Deposit some `toCollateral` into the vault so it has balance to send out later
          const initialVaultToCollateral = hre.ethers.parseUnits("1000", toInfo.decimals);
          await toContract.approve(await collateralVaultContract.getAddress(), initialVaultToCollateral);
          await collateralVaultContract.deposit(initialVaultToCollateral, toInfo.address);
          ...
          const isSupported = await collateralVaultContract.isCollateralSupported(fromInfo.address);
          ...
          const COLLATERAL_STRATEGY_ROLE = await collateralVaultContract.COLLATERAL_STRATEGY_ROLE();
          await collateralVaultContract.grantRole(COLLATERAL_STRATEGY_ROLE, user1);
        });
        ...
        it("allows swapping unsupported vault collateral for supported collateral in", async function () {
          ...
          const vaultFromBefore = await fromContract.balanceOf(await collateralVaultContract.getAddress());
          const vaultToBefore = await toContract.balanceOf(await collateralVaultContract.getAddress());

          // Execute exchange
          await collateralVaultContract
            .connect(await hre.ethers.getSigner(user1))
            .exchangeCollateral(fromAmount, fromInfo.address, toAmount, toInfo.address);

          // Balances after
          const userFromAfter = await fromContract.balanceOf(user1);
          const userToAfter = await toContract.balanceOf(user1);
          const vaultFromAfter = await fromContract.balanceOf(await collateralVaultContract.getAddress());
          const vaultToAfter = await toContract.balanceOf(await collateralVaultContract.getAddress());

          // Assertions
          assert.equal(userFromBefore - userFromAfter, fromAmount, "User should have sent fromCollateral");
          assert.equal(userToAfter - userToBefore, toAmount, "User should have received toCollateral");
          assert.equal(vaultFromAfter - vaultFromBefore, fromAmount, "Vault should have received fromCollateral");
          assert.equal(vaultToBefore - vaultToAfter, toAmount, "Vault should have sent out toCollateral");
        });
      });
    }
  });
