...
export const createOracleAggregatorFixture = (config: OracleAggregatorFixtureConfig) => {
  return deployments.createFixture(async ({ deployments, getNamedAccounts, ethers }): Promise<OracleAggregatorFixtureResult> => {
    ...
    const { address: redstoneChainlinkWrapperAddress } = await deployments.get(config.wrapperIds.redstoneChainlinkWrapper);
    const redstoneChainlinkWrapper = await ethers.getContractAt("RedstoneChainlinkWrapper", redstoneChainlinkWrapperAddress);
    ...
    for (const [address, data] of Object.entries(config.redstoneOracleAssets.redstoneOracleWrappersWithThresholding)) {
    ...
    for (const [address, data] of Object.entries(config.redstoneOracleAssets.compositeRedstoneOracleWrappersWithThresholding)) {
...
export const getOracleAggregatorFixture = async (currency: string) => {
  ...
  const fixtureConfig: OracleAggregatorFixtureConfig = {
    ...
    wrapperIds: {
      ...
      api3WrapperWithThresholding: currency === "USD" ? USD_API3_WRAPPER_WITH_THRESHOLDING_ID : S_API3_WRAPPER_WITH_THRESHOLDING_ID,
      api3CompositeWrapperWithThresholding:
        currency === "USD" ? USD_API3_COMPOSITE_WRAPPER_WITH_THRESHOLDING_ID : S_API3_COMPOSITE_WRAPPER_WITH_THRESHOLDING_ID,
      ...
      redstoneChainlinkCompositeWrapperWithThresholding:
        currency === "USD" ? USD_REDSTONE_COMPOSITE_WRAPPER_WITH_THRESHOLDING_ID : S_REDSTONE_COMPOSITE_WRAPPER_WITH_THRESHOLDING_ID,
    },
  };
