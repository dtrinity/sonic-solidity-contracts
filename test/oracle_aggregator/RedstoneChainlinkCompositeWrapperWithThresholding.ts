...
async function runTestsForCurrency(currency: string, { deployer, user1, user2 }: { deployer: Address; user1: Address; user2: Address }) {
  describe(`RedstoneChainlinkCompositeWrapperWithThresholding for ${currency}`, () => {
    ...
    beforeEach(async function () {
      ...
      const oracleManagerRole = await redstoneChainlinkCompositeWrapperWithThresholding.ORACLE_MANAGER_ROLE();
      await redstoneChainlinkCompositeWrapperWithThresholding.grantRole(oracleManagerRole, deployer);
    });

    describe("Base currency and units", () => {
      it("should return correct BASE_CURRENCY", async function () {
        const baseCurrency = await redstoneChainlinkCompositeWrapperWithThresholding.BASE_CURRENCY();
      ...
      it("should return correct BASE_CURRENCY_UNIT", async function () {
        const actualUnit = await redstoneChainlinkCompositeWrapperWithThresholding.BASE_CURRENCY_UNIT();
    ...
    describe("Asset pricing with composite thresholding", () => {
      it("should correctly price composite assets", async function () {
        for (const [address, asset] of Object.entries(fixtureResult.assets.redstoneCompositeAssets)) {
          const { price, isAlive } = await redstoneChainlinkCompositeWrapperWithThresholding.getPriceInfo(address);

          expect(price).to.be.gt(0);
          expect(isAlive).to.be.true;

          const directPrice = await redstoneChainlinkCompositeWrapperWithThresholding.getAssetPrice(address);
          expect(directPrice).to.equal(price);
        }
      });

      it("should handle thresholding for both primary and secondary prices", async function () {
        const testAsset = getRandomItemFromList(Object.keys(fixtureResult.assets.redstoneCompositeAssets));

        // Deploy mock feeds for testing
        const mockFeed1 = await ethers.deployContract("MockRedstoneChainlinkOracleAlwaysAlive");
        const mockFeed2 = await ethers.deployContract("MockRedstoneChainlinkOracleAlwaysAlive");

        // Set up composite feed with thresholds
        const lowerThreshold1 = ethers.parseUnits("0.99", ORACLE_AGGREGATOR_PRICE_DECIMALS);
        const fixedPrice1 = ethers.parseUnits("1.00", ORACLE_AGGREGATOR_PRICE_DECIMALS);
        const lowerThreshold2 = ethers.parseUnits("0.98", ORACLE_AGGREGATOR_PRICE_DECIMALS);
        const fixedPrice2 = ethers.parseUnits("1.00", ORACLE_AGGREGATOR_PRICE_DECIMALS);
        ...
        const price1Above = ethers.parseUnits("1.02", ORACLE_AGGREGATOR_PRICE_DECIMALS);
        const price2Above = ethers.parseUnits("1.05", ORACLE_AGGREGATOR_PRICE_DECIMALS);
        ...
        const price1Below = ethers.parseUnits("0.95", ORACLE_AGGREGATOR_PRICE_DECIMALS);
      ...
      it("should revert when getting price for non-existent asset", async function () {
        ...
        await expect(redstoneChainlinkCompositeWrapperWithThresholding.getPriceInfo(nonExistentAsset))
          .to.be.revertedWithCustomError(redstoneChainlinkCompositeWrapperWithThresholding, "FeedNotSet")
          .withArgs(nonExistentAsset);

        await expect(redstoneChainlinkCompositeWrapperWithThresholding.getAssetPrice(nonExistentAsset))
          .to.be.revertedWithCustomError(redstoneChainlinkCompositeWrapperWithThresholding, "FeedNotSet")
          .withArgs(nonExistentAsset);
      });
    });
    ...
    describe("Feed management", () => {
      it("should allow adding and removing composite feeds", async function () {
        ...
        await expect(
          ...
          .to.emit(redstoneChainlinkCompositeWrapperWithThresholding, "CompositeFeedAdded")
          .withArgs(testAsset, feed1, feed2, lowerThreshold1, fixedPrice1, lowerThreshold2, fixedPrice2);

        // Verify feed configuration
        const feed = await redstoneChainlinkCompositeWrapperWithThresholding.compositeFeeds(testAsset);
        expect(feed.feed1).to.equal(feed1);
        expect(feed.feed2).to.equal(feed2);
        expect(feed.primaryThreshold.lowerThresholdInBase).to.equal(lowerThreshold1);
        expect(feed.primaryThreshold.fixedPriceInBase).to.equal(fixedPrice1);
        expect(feed.secondaryThreshold.lowerThresholdInBase).to.equal(lowerThreshold2);
        expect(feed.secondaryThreshold.fixedPriceInBase).to.equal(fixedPrice2);

        // Remove feed
        await expect(redstoneChainlinkCompositeWrapperWithThresholding.removeCompositeFeed(testAsset))
          .to.emit(redstoneChainlinkCompositeWrapperWithThresholding, "CompositeFeedRemoved")
          .withArgs(testAsset);

        // Verify feed is removed
        const removedFeed = await redstoneChainlinkCompositeWrapperWithThresholding.compositeFeeds(testAsset);
