...
async function runTestsForCurrency(
  ...
  describe(`RedstoneChainlinkWrapperWithThresholding for ${currency}`, () => {
    ...
    beforeEach(async function () {
      ...
      redstoneChainlinkWrapperWithThresholding = fixtureResult.contracts.redstoneChainlinkWrapperWithThresholding;
      ...
      await redstoneChainlinkWrapperWithThresholding.grantRole(oracleManagerRole, deployer);
    });

    describe("Base currency and units", () => {
      it("should return correct BASE_CURRENCY", async function () {
        const baseCurrency = await redstoneChainlinkWrapperWithThresholding.BASE_CURRENCY();
      ...
      it("should return correct BASE_CURRENCY_UNIT", async function () {
        const actualUnit = await redstoneChainlinkWrapperWithThresholding.BASE_CURRENCY_UNIT();
    ...
    describe("Asset pricing with thresholding", () => {
      it("should return original price when no threshold is set", async function () {
        ...
        const testAsset = getRandomItemFromList(Object.keys(fixtureResult.assets.redstoneThresholdAssets));
        const assetData = fixtureResult.assets.redstoneThresholdAssets[testAsset];

        // Get the existing MockRedstoneChainlinkOracleAlwaysAlive instance from the fixture data
        const mockFeed = await ethers.getContractAt("MockRedstoneChainlinkOracleAlwaysAlive", assetData.feed);

        // Set a test price
        const testPrice = ethers.parseUnits("1", ORACLE_AGGREGATOR_PRICE_DECIMALS);
        ...
        const directPrice = await redstoneChainlinkWrapperWithThresholding.getAssetPrice(testAsset);
      ...
      it("should return fixed price when price is above threshold", async function () {
        ...
        for (const [testAsset, assetData] of Object.entries(fixtureResult.assets.redstoneThresholdAssets)) {
          // Get the existing MockRedstoneChainlinkOracleAlwaysAlive instance from the fixture data
          const mockFeed = await ethers.getContractAt("MockRedstoneChainlinkOracleAlwaysAlive", assetData.feed);
          ...
          // Verify getAssetPrice returns the same value
          const directPrice = await redstoneChainlinkWrapperWithThresholding.getAssetPrice(testAsset);
          expect(directPrice).to.equal(assetData.fixedPrice, `Asset: ${testAsset}`);
        }
      });
      ...
      it("should return original price when price is below threshold", async function () {
        ...
        for (const [testAsset, assetData] of Object.entries(fixtureResult.assets.redstoneThresholdAssets)) {
          ...
          const priceBelowThreshold18Decimals = assetData.lowerThreshold - assetData.lowerThreshold / 100n;
          const currentBlock = await ethers.provider.getBlock("latest");
          ...
          // Convert the target price to 8 decimals for the mock feed
          const mockPrice8Decimals =
            (priceBelowThreshold18Decimals * BigInt(10) ** BigInt(CHAINLINK_FEED_DECIMALS)) /
            BigInt(10) ** BigInt(ORACLE_AGGREGATOR_PRICE_DECIMALS);
          ...
          // Verify getAssetPrice returns the same value
          const directPrice = await redstoneChainlinkWrapperWithThresholding.getAssetPrice(testAsset);
          expect(directPrice).to.equal(priceBelowThreshold18Decimals, `Asset: ${testAsset}`);
        }
      });
    });
    ...
    describe("Threshold configuration management", () => {
      it("should allow setting and removing threshold config", async function () {
        ...
        const lowerThreshold = ethers.parseUnits("0.99", ORACLE_AGGREGATOR_PRICE_DECIMALS);
        const fixedPrice = ethers.parseUnits("1.00", ORACLE_AGGREGATOR_PRICE_DECIMALS);

        await expect(redstoneChainlinkWrapperWithThresholding.setThresholdConfig(testAsset, lowerThreshold, fixedPrice))
          .to.emit(redstoneChainlinkWrapperWithThresholding, "ThresholdConfigSet")
          .withArgs(testAsset, lowerThreshold, fixedPrice);

        // Verify config
        const config = await redstoneChainlinkWrapperWithThresholding.assetThresholds(testAsset);
        ...
        await expect(redstoneChainlinkWrapperWithThresholding.removeThresholdConfig(testAsset))
          .to.emit(redstoneChainlinkWrapperWithThresholding, "ThresholdConfigRemoved")
          .withArgs(testAsset);

        // Verify config is removed
        const removedConfig = await redstoneChainlinkWrapperWithThresholding.assetThresholds(testAsset);
