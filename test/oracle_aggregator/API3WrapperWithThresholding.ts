...
import { API3WrapperWithThresholding } from "../../typechain-types";
...
async function runTestsForCurrency(
  ...
  describe(`API3WrapperWithThresholding for ${currency}`, () => {
    ...
    beforeEach(async function () {
      ...
      api3WrapperWithThresholding = fixtureResult.contracts.api3WrapperWithThresholding;
    ...
    describe("Base currency and units", () => {
      ...
      it("should return correct BASE_CURRENCY_UNIT", async function () {
        const actualUnit = await api3WrapperWithThresholding.BASE_CURRENCY_UNIT();
    ...
    describe("Asset pricing with thresholding", () => {
      it("should return original price when no threshold is set", async function () {
        ...
        const testAsset = getRandomItemFromList(Object.keys(fixtureResult.assets.api3ThresholdAssets));

        // Get price info
        const { price: actualPrice, isAlive } = await api3WrapperWithThresholding.getPriceInfo(testAsset);
        ...
        const directPrice = await api3WrapperWithThresholding.getAssetPrice(testAsset);
      ...
      it("should return fixed price when price is above threshold", async function () {
        ...
        for (const [testAsset, assetData] of Object.entries(fixtureResult.assets.api3ThresholdAssets)) {
      ...
      it("should return original price when price is below threshold", async function () {
        ...
        for (const [testAsset, assetData] of Object.entries(fixtureResult.assets.api3ThresholdAssets)) {
          ...
          expect(actualPrice).to.equal(priceBelowThreshold, `Asset: ${testAsset}`);
          expect(isAlive).to.be.true;

          // Verify getAssetPrice returns the same value
          const directPrice = await api3WrapperWithThresholding.getAssetPrice(testAsset);
          expect(directPrice).to.equal(priceBelowThreshold, `Asset: ${testAsset}`);
        }
      });

      it("should handle stale prices correctly", async function () {
        // Get a threshold asset (if available)
        const thresholdAssets = Object.keys(fixtureResult.assets.api3ThresholdAssets);
        ...
        const price = assetData.lowerThreshold - assetData.lowerThreshold / 100n;
        const currentBlock = await ethers.provider.getBlock("latest");
        ...
        const { isAlive } = await api3WrapperWithThresholding.getPriceInfo(testAsset);
        expect(isAlive).to.be.false;

        // getAssetPrice should revert
        await expect(api3WrapperWithThresholding.getAssetPrice(testAsset)).to.be.revertedWithCustomError(
    ...
    describe("Threshold configuration management", () => {
      it("should allow setting and removing threshold config", async function () {
        ...
        await expect(api3WrapperWithThresholding.setThresholdConfig(testAsset, lowerThreshold, fixedPrice))
          .to.emit(api3WrapperWithThresholding, "ThresholdConfigSet")
          .withArgs(testAsset, lowerThreshold, fixedPrice);

        // Verify config
        const config = await api3WrapperWithThresholding.assetThresholds(testAsset);
        ...
        await expect(api3WrapperWithThresholding.removeThresholdConfig(testAsset))
          .to.emit(api3WrapperWithThresholding, "ThresholdConfigRemoved")
          .withArgs(testAsset);

        // Verify config is removed
        const removedConfig = await api3WrapperWithThresholding.assetThresholds(testAsset);
      ...
      it("should revert when non-ORACLE_MANAGER tries to remove threshold config", async function () {
        ...
        await expect(api3WrapperWithThresholding.connect(unauthorizedSigner).removeThresholdConfig(testAsset))
          .to.be.revertedWithCustomError(api3WrapperWithThresholding, "AccessControlUnauthorizedAccount")
          .withArgs(user2, oracleManagerRole);
      });
    });
