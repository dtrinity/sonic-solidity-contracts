...
describe("dLEND AaveProtocolDataProvider", () => {
  ...
  beforeEach(async () => {
    ...
    collateralAsset = Object.keys(fixture.assets).find((asset) => fixture.assets[asset].ltv !== BigInt(0))!;

    if (!dStableAsset || !collateralAsset) {
      throw new Error("Could not find required test assets in fixture: need dStable and collateral");
    }
  ...
  describe("Reserve Configuration Data (`getReserveConfigurationData`)", () => {
    it("should return correct configuration data for a collateral reserve", async () => {
      ...
      const token = await hre.ethers.getContractAt("TestERC20", collateralAsset);
      const decimals = await token.decimals();

      expect(config.decimals).to.equal(decimals);
      expect(config.ltv).to.equal(assetInfo.ltv);
      expect(config.liquidationThreshold).to.equal(assetInfo.liquidationThreshold);
    ...
    it("should return correct configuration data for a dStable reserve", async () => {
      const config = await dataProvider.getReserveConfigurationData(dStableAsset);
      const assetInfo = fixture.assets[dStableAsset];
      const token = await hre.ethers.getContractAt("ERC20StablecoinUpgradeable", dStableAsset);
      const decimals = await token.decimals();
  ...
  describe("Reserve Token Addresses (`getReserveTokensAddresses`)", () => {
    it("should return correct token addresses for a collateral reserve", async () => {
      const addresses = await dataProvider.getReserveTokensAddresses(collateralAsset);
      const assetInfo = fixture.assets[collateralAsset];

      expect(addresses.aTokenAddress).to.equal(assetInfo.aToken);
      expect(addresses.stableDebtTokenAddress).to.equal(assetInfo.stableDebtToken);
      expect(addresses.variableDebtTokenAddress).to.equal(assetInfo.variableDebtToken);
    });

    it("should return correct token addresses for a dStable reserve", async () => {
      const addresses = await dataProvider.getReserveTokensAddresses(dStableAsset);
      const assetInfo = fixture.assets[dStableAsset];
  ...
  describe("All Reserves Data (`getAllReservesTokens`, `getAllATokens`)", () => {
    it("should return correct data for all reserves", async () => {
      ...
      const collateralReserve = allReserves.find((r) => r.tokenAddress === collateralAsset);
      expect(collateralReserve).to.not.be.undefined;
      expect(collateralReserve?.symbol).to.equal(fixture.assets[collateralAsset].symbol);

      // Verify a known dStable asset is present
      const dStableReserve = allReserves.find((r) => r.tokenAddress === dStableAsset);
      expect(dStableReserve).to.not.be.undefined;
      expect(dStableReserve?.symbol).to.equal(fixture.assets[dStableAsset].symbol);
    });

    it("should return correct data for all aTokens", async () => {
      const allATokens = await dataProvider.getAllATokens();
      const expectedATokensCount = Object.keys(fixture.contracts.aTokens).length;

      expect(allATokens.length).to.equal(expectedATokensCount);

      // Verify a known collateral aToken is present
      const collateralATokenInfo = allATokens.find((t) => t.tokenAddress === fixture.assets[collateralAsset].aToken);
      ...
      const dStableATokenInfo = allATokens.find((t) => t.tokenAddress === fixture.assets[dStableAsset].aToken);
  ...
  describe("Aggregated Reserve Data (`getReserveData`)", () => {
    it("should return correct aggregated data for a dStable reserve with initial liquidity", async () => {
      ...
      const dUsdToken = await hre.ethers.getContractAt("TestERC20", dStableAsset);
      ...
      expect(totalStableDebt).to.equal(0, "Value at index 3 (totalStableDebt) should be 0");
      expect(totalVariableDebt).to.equal(0, "Value at index 4 (totalVariableDebt) should be 0");

      expect(liquidityRate).to.be.gte(0, "Value at index 5 (liquidityRate) should be >= 0");
      expect(variableBorrowRate).to.be.gte(0, "Value at index 6 (variableBorrowRate) should be >= 0");
      expect(stableBorrowRate).to.be.gte(0, "Value at index 7 (stableBorrowRate) should be >= 0");
    });

    it("should return correct aggregated data for a collateral reserve with no initial liquidity (except deployer)", async () => {
      const [unbacked, accruedToTreasuryScaled, totalAToken, totalStableDebt, totalVariableDebt] =
  ...
  describe("User Reserve Data (`getUserReserveData`)", () => {
    it("should return correct data for a user who supplied initial liquidity", async () => {
      const userData = await dataProvider.getUserReserveData(dStableAsset, deployerSigner.address);
      const aToken = fixture.contracts.aTokens[dStableAsset];
      const expectedATokenBalance = await aToken.balanceOf(deployerSigner.address);
    ...
    it("should return zero values for a user with no position in a reserve", async () => {
      const userData = await dataProvider.getUserReserveData(collateralAsset, user1Signer.address);
