...
describe("dLEND UiPoolDataProviderV3", () => {
  ...
  beforeEach(async () => {
    ...
    const { contract: wSToken } = await getTokenContractForSymbol(hre, deployer, "wS");

    // Deploy UiPoolDataProviderV3 with the oracle and wS token address
    const UiPoolDataProviderV3Factory = await ethers.getContractFactory("UiPoolDataProviderV3");
    uiPoolDataProvider = await UiPoolDataProviderV3Factory.deploy(priceOracleAddress, await wSToken.getAddress());

    // Get test assets from the fixture's assets mapping
    const assets = Object.entries(fixture.assets);
    dStableAsset = assets.find(([_, config]) => config.symbol === "dUSD")?.[0] || "";
    collateralAsset = assets.find(([_, config]) => config.ltv.toString() !== "0")?.[0] || "";
  ...
  describe("getReservesList", () => {
    it("should return the list of all reserve assets matching the fixture", async () => {
      const reservesList = await uiPoolDataProvider.getReservesList(addressesProvider);
  ...
  describe("getReservesData", () => {
    it("should return aggregated data for all reserves", async () => {
      const [reservesData, baseCurrencyInfo] = await uiPoolDataProvider.getReservesData(addressesProvider);
      const fixtureAssets = Object.keys(fixture.assets);

      expect(reservesData.length).to.equal(fixtureAssets.length);
      expect(reservesData.map((r) => r.underlyingAsset)).to.have.members(fixtureAssets);
    });

    it("should return correct base currency information", async () => {
      const [_, baseCurrencyInfo] = await uiPoolDataProvider.getReservesData(addressesProvider);

      expect(baseCurrencyInfo.marketReferenceCurrencyUnit).to.equal(10 ** 8);
      expect(baseCurrencyInfo.marketReferenceCurrencyPriceInUsd).to.not.equal(0);
    ...
    it("should return correct data points for a collateral asset", async () => {
      const [reservesData] = await uiPoolDataProvider.getReservesData(addressesProvider);
      const collateralData = reservesData.find((r) => r.underlyingAsset === collateralAsset);
      ...
      expect(collateralData!.baseLTVasCollateral).to.equal(fixtureAssetInfo.ltv);
      expect(collateralData!.reserveLiquidationThreshold).to.equal(fixtureAssetInfo.liquidationThreshold);
      expect(collateralData!.aTokenAddress).to.equal(fixtureAssetInfo.aToken);
      expect(collateralData!.variableDebtTokenAddress).to.equal(fixtureAssetInfo.variableDebtToken);
      expect(collateralData!.stableDebtTokenAddress).to.equal(fixtureAssetInfo.stableDebtToken);
    });

    it("should return correct data points for a dStable asset", async () => {
      const [reservesData] = await uiPoolDataProvider.getReservesData(addressesProvider);
      const dStableData = reservesData.find((r) => r.underlyingAsset === dStableAsset);
  ...
  describe("getUserReservesData", () => {
    it("should return correct data for a user with deposits and borrows", async () => {
      ...
      const collateralToken = await ethers.getContractAt("TestERC20", collateralAsset);

      // Transfer tokens from deployer to user1 instead of minting
      await collateralToken.connect(deployerSigner).transfer(user1Signer.address, collateralAmount);
