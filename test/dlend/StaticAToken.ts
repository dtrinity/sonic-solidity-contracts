...
describe("StaticATokenFactory & StaticATokenLM", () => {
  ...
  beforeEach(async () => {
    ...
    const Factory = await ethers.getContractFactory("StaticATokenFactory", deployer);
    factory = (await Factory.deploy(await pool.getAddress())) as StaticATokenFactory;
  });
  ...
  describe("createStaticATokens", () => {
    ...
    beforeEach(async () => {
      const nonDStable = Object.values(fixture.assets).find((a) => !a.isDStable);
    ...
    it("factory starts empty", async () => {
      ...
      expect(await factory.getStaticAToken(nonDStableAsset)).to.equal(ethers.ZeroAddress);
    });
    ...
    it("deploys new wrapper for non-dStable and caches it", async () => {
      ...
      await expect(tx).to.emit(factory, "StaticTokenCreated").withArgs(tokenAddr, nonDStableAsset);
      expect(await factory.getStaticATokens()).to.deep.equal([tokenAddr]);

      const staticToken = (await ethers.getContractAt("StaticATokenLM", tokenAddr)) as StaticATokenLM;
      expect(await staticToken.name()).to.equal("Wrapped " + (await nonDStableAToken.name()));
      expect(await staticToken.symbol()).to.equal("w" + (await nonDStableAToken.symbol()));
      expect(await staticToken.decimals()).to.equal(await nonDStableAToken.decimals());

      const tx2 = await factory.createStaticATokens([nonDStableAsset]);
      await expect(tx2).not.to.emit(factory, "StaticTokenCreated");
      expect(await factory.getStaticAToken(nonDStableAsset)).to.equal(tokenAddr);
      expect(await factory.getStaticATokens()).to.deep.equal([tokenAddr]);
    });

    it("reverts for unlisted asset", async () => {
      await expect(factory.createStaticATokens([ethers.ZeroAddress])).to.be.revertedWith("UNDERLYING_NOT_LISTED");
    });

    it("batch deploys multiple wrappers (incl. dS)", async () => {
      const assetsToDeploy: string[] = [];
      const dSAddress = fixture.dStables.dS;
      if (!nonDStableAsset) throw new Error("nonDStableAsset not set for batch test");
      ...
      expect(finalRegistry.length).to.equal(initialRegistry.length + assetsToDeploy.length);
  ...
  describe("deposit/withdraw & rebasing (wrapping dS)", () => {
    ...
    beforeEach(async () => {
      ...
      staticToken = await ethers.getContractAt("StaticATokenLM", tokenAddr, deployer);
      ...
      const collateralAssets = Object.values(fixture.assets).filter((a) => !a.isDStable && a.ltv !== BigInt(0));
      if (collateralAssets.length === 0) throw new Error("Need a collateral asset for the borrower.");
      ...
      user2CollateralToken = await ethers.getContractAt("TestERC20", user2CollateralAsset);
      const user2CollateralDecimals = await user2CollateralToken.decimals();

      const dSPrice: bigint = await fixture.contracts.priceOracle.getAssetPrice(underlying);
      const collateralPrice: bigint = await fixture.contracts.priceOracle.getAssetPrice(user2CollateralAsset);
      ...
      const requiredCollateralValueBase: bigint = (borrowValueBase * tenPow4) / collateralLTV;
      ...
      await user2CollateralToken.connect(deployer).transfer(user2.address, user2CollateralAmount);
      await user2CollateralToken.connect(user2).approve(poolAddress, user2CollateralAmount);
      await pool.connect(user2).supply(user2CollateralAsset, user2CollateralAmount, user2.address, 0);
      await pool.connect(user2).setUserUseReserveAsCollateral(user2CollateralAsset, true);

      await underlyingToken.connect(deployer).approve(staticTokenAddress, depositAmount);
      await (staticToken as any).deposit(depositAmount, deployer.address);

      await pool.connect(user2).borrow(underlying, borrowAmount, 2, 0, user2.address);

      await underlyingToken.connect(deployer).transfer(user3.address, user3DepositAmount);
      await underlyingToken.connect(user3).approve(poolAddress, user3DepositAmount);
      await pool.connect(user3).supply(underlying, user3DepositAmount, user3.address, 0);
    });

    it("can deposit and withdraw dS via wrapper", async () => {
      expect(await staticToken.balanceOf(deployer.address)).to.equal(depositAmount);
      ...
      const expectedUnderlyingOut = await staticToken.previewRedeem(depositAmount);
      const deployerUnderlyingBalanceBefore = await underlyingToken.balanceOf(deployer.address);
      const deployerATokenBalanceBefore = await aToken.balanceOf(deployer.address);
      const wrapperATokenBalanceBefore = await aToken.balanceOf(staticTokenAddress);

      await (staticToken as any).redeem(depositAmount, deployer.address, deployer.address);

      const deployerUnderlyingBalanceAfter = await underlyingToken.balanceOf(deployer.address);
      const receivedUnderlying = deployerUnderlyingBalanceAfter - deployerUnderlyingBalanceBefore;
      const tolerance1 = expectedUnderlyingOut / 1_000_000n + 1n;
      expect(receivedUnderlying).to.be.closeTo(expectedUnderlyingOut, tolerance1);

      expect(await staticToken.balanceOf(deployer.address)).to.equal(0);
      const wrapperATokenBalanceAfter = await aToken.balanceOf(staticTokenAddress);
      ...
      expect(finalATokenBalanceCheck).to.be.closeTo(deployerATokenBalanceBefore, 1);
    ...
    it("rebasing increases assets per share without changing share balance", async () => {
      ...
      await underlyingToken.connect(deployer).transfer(user2.address, interactionAmount);
      await underlyingToken.connect(user2).approve(poolAddress, interactionAmount);
      await pool.connect(user2).repay(underlying, interactionAmount, 2, user2.address);
    ...
    it("withdraw returns correct amount of aTokens after rebase", async () => {
      ...
      const assetsValueBeforeRedeem = await staticToken.convertToAssets(initialShares);
      ...
      await (staticToken as any).redeemATokens(initialShares, deployer.address, deployer.address);
    ...
    it("withdraw underlying returns correct amount after rebase", async () => {
      ...
      const expectedWithdrawAmountCheck = await staticToken.previewRedeem(maxSharesRedeemable);
      expect(expectedWithdrawAmountCheck).to.be.gt(depositAmount);

      const previewAmount = await staticToken.previewRedeem(maxSharesRedeemable);

      const underlyingBalanceBefore_Test2 = await underlyingToken.balanceOf(deployer.address);
      const deployerATokenBalanceBefore_Test2 = await aToken.balanceOf(deployer.address);
      const wrapperATokenBalanceBefore_Test2 = await aToken.balanceOf(staticTokenAddress);

      await (staticToken as any).redeem(maxSharesRedeemable, deployer.address, deployer.address);

      const underlyingBalanceAfter_Test2 = await underlyingToken.balanceOf(deployer.address);
      const receivedAmount = underlyingBalanceAfter_Test2 - underlyingBalanceBefore_Test2;
      const tolerance4 = previewAmount / 1_000_000n + 1n;

      expect(receivedAmount).to.be.closeTo(previewAmount, tolerance4);
      expect(await staticToken.balanceOf(deployer.address)).to.equal(initialShares - maxSharesRedeemable);
      // Check Deployer aToken Balance (Should NOT change)
      const finalATokenBalanceCheck_Test2 = await aToken.balanceOf(deployer.address);
      expect(finalATokenBalanceCheck_Test2).to.be.closeTo(deployerATokenBalanceBefore_Test2, 1);
      // Check Wrapper aToken Balance (Should DECREASE by underlying value withdrawn)
    ...
    it("withdraw non-Aave returns correct amount of aTokens", async () => {
      ...
      const assetsToWithdraw = await staticToken.previewRedeem(sharesToWithdraw);
      ...
      const deployerStaticBalanceBefore = await staticToken.balanceOf(deployer.address);
      const deployerATokenBalanceBefore = await aToken.balanceOf(deployer.address);
      const wrapperATokenBalanceBefore = await aToken.balanceOf(staticTokenAddress);

      await staticToken.connect(deployer).withdraw(assetsToWithdraw, deployer.address, deployer.address);

      const deployerStaticBalanceAfter = await staticToken.balanceOf(deployer.address);
      const actualSharesBurned = deployerStaticBalanceBefore - deployerStaticBalanceAfter;
      ...
      const wrapperATokenBalanceAfter = await aToken.balanceOf(staticTokenAddress);
      const expectedWrapperBalanceAfter = wrapperATokenBalanceBefore - assetsToWithdraw; // It decreases by the underlying value withdrawn
      const wrapperTolerance = expectedWrapperBalanceAfter / 1_000_000n + 1n; // Use relative tolerance on expected value
      expect(wrapperATokenBalanceAfter).to.be.closeTo(expectedWrapperBalanceAfter, wrapperTolerance);
    });
    ...
    it("withdraw(assets) returns correct amount of underlying", async () => {
      ...
      const deployerStaticBalanceBefore = await staticToken.balanceOf(deployer.address);
      const deployerATokenBalanceBefore_Test3 = await aToken.balanceOf(deployer.address);
      const wrapperATokenBalanceBefore_Test3 = await aToken.balanceOf(staticTokenAddress);
      const deployerUnderlyingBalanceBefore = await underlyingToken.balanceOf(deployer.address);
      ...
      const deployerATokenBalanceAfter_Test3 = await aToken.balanceOf(deployer.address);
      expect(deployerATokenBalanceAfter_Test3).to.be.closeTo(deployerATokenBalanceBefore_Test3, 1);
      ...
      expect(receivedUnderlying).to.be.closeTo(assetsToWithdraw, underlyingTolerance);
    });
    ...
    it("can deposit and redeem aTokens via depositATokens/redeemATokens", async () => {
      ...
      const initialStaticBalance = await staticToken.balanceOf(deployer.address);
      ...
      if (deployerATokenStart > 0n) {
        ...
        await aToken.connect(deployer).transfer(user1.address, deployerATokenStart);
      }
      expect(await aToken.balanceOf(deployer.address)).to.equal(0);

      await underlyingToken.connect(deployer).approve(poolAddress, depositAmount);
      await pool.connect(deployer).supply(underlying, depositAmount, deployer.address, 0); // aTokens sent to deployer

      // Calculate exact aTokens received from this supply call
      const deployerATokenAfterSupply = await aToken.balanceOf(deployer.address);
      ...
      await aToken.connect(deployer).approve(staticTokenAddress, aTokenAmountReceived);
      const expectedSharesMinted = await staticToken.previewDeposit(aTokenAmountReceived);
      const staticBalanceBeforeDeposit = await staticToken.balanceOf(deployer.address);
      const wrapperATokenBalanceBeforeDeposit = await aToken.balanceOf(staticTokenAddress);

      await (staticToken as any).depositATokens(aTokenAmountReceived, deployer.address);

      // --- Step 3: Verify state after depositATokens ---
      const staticBalanceAfterDeposit = await staticToken.balanceOf(deployer.address);
      const actualSharesMinted = staticBalanceAfterDeposit - staticBalanceBeforeDeposit;
      ...
      expect(staticBalanceAfterDeposit).to.be.closeTo(initialStaticBalance + expectedSharesMinted, tolerance);
      ...
      const staticBalanceBeforeRedeem = await staticToken.balanceOf(deployer.address); // Should be initialStaticBalance + actualSharesMinted
      const expectedAssetsOut = await staticToken.previewRedeem(actualSharesMinted); // Calculate aTokens expected for the shares minted IN THIS TEST
    ...
    it("depositATokens vs deposit underlying: minted shares are comparable", async () => {
      ...
      await underlyingToken.connect(deployer).approve(poolAddress, depositAmount);
      await pool.connect(deployer).supply(underlying, depositAmount, deployer.address, 0);
      const aTokenAmountReceived = await aToken.balanceOf(deployer.address);

      // Deposit aTokens
      await aToken.connect(deployer).approve(staticTokenAddress, aTokenAmountReceived);
      await (staticToken as any).depositATokens(aTokenAmountReceived, deployer.address);
      const balanceAfterATokenDeposit = await staticToken.balanceOf(deployer.address);
      ...
      if (totalStaticBalance > 0) {
        ...
        await staticToken.connect(deployer).approve(staticTokenAddress, totalStaticBalance);
        await (staticToken as any).redeem(totalStaticBalance, deployer.address, deployer.address);
      }
      ...
      await underlyingToken.connect(deployer).approve(staticTokenAddress, depositAmount);
      await (staticToken as any).deposit(depositAmount, deployer.address);
      const sharesFromUnderlying = await staticToken.balanceOf(deployer.address);
      ...
      if (sharesFromUnderlying > 0) {
        await staticToken.connect(deployer).approve(staticTokenAddress, sharesFromUnderlying);
        await (staticToken as any).redeem(sharesFromUnderlying, deployer.address, deployer.address);
      }
    });
  });
});
