...
export interface OdosV2TestFixture {
  ...
  pool: MockPoolV2;
  priceOracle: MockPriceOracleGetter;
}
...
export async function deployOdosV2TestFixture(): Promise<OdosV2TestFixture> {
  ...
  const MockPriceOracleGetterFactory = await ethers.getContractFactory("MockPriceOracleGetter");
  const priceOracle = await MockPriceOracleGetterFactory.deploy();
