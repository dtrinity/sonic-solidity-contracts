...
describe("SwapExecutorV2 - Pure Logic Tests", function () {
  ...
  describe("Route Determination Logic", function () {
    ...
    it("should validate PT swap data before routing", async function () {
      ...
      await expect(
        swapExecutorHarness.executeSwapExactInput(params)
      ).to.be.revertedWithCustomError(swapExecutorHarness, "InvalidSwapData");
    });
  });
  ...
  describe("State Consistency Logic", function () {
    ...
    it("should handle concurrent validation calls", async function () {
      ...
      const type1 = await pendleLogicHarness.determineSwapType(
        await tokenA.getAddress(),
        await tokenB.getAddress()
      );
      
      const type2 = await pendleLogicHarness.determineSwapType(
