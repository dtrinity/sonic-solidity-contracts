...
describe("BaseOdosSellAdapter", function () {
  /**
   *
   */
  ...
  it("executes happy-path sell with correct amounts and event", async function () {
    ...
    const amountSpent = parseUnits("1000", 18); // Router will spend exactly what we ask
    const amountReceived = parseUnits("2000", 18);
    const minAmountToReceive = parseUnits("1900", 18);
  ...
  it("reverts when adapter has insufficient balance", async function () {
    ...
    const amountToSwap = parseUnits("1500", 18);
    const amountSpent = parseUnits("1500", 18);
    const amountReceived = parseUnits("2000", 18);
    const minAmountToReceive = parseUnits("1900", 18);
  ...
  it("reverts when router delivers less than minAmountToReceive", async function () {
    ...
    const amountReceived = parseUnits("1500", 18); // Less than minimum
    const minAmountToReceive = parseUnits("2000", 18); // Higher than what router delivers
