...
describe("RewardClaimable", function () {
  ...
  beforeEach(async function () {
    ...
    exchangeAsset = (await RewardClaimableMockERC20Factory.connect(admin).deploy(
      "Exchange Asset",
      "EA",
  ...
  describe("Treasury Management", function () {
    ...
    it("Should revert if treasury is zero address", async function () {
      await expect(mockVault.connect(admin).setTreasury(ethers.ZeroAddress)).to.be.revertedWithCustomError(
        mockVault,
        "ZeroTreasuryAddress",
      );
    });
  ...
  describe("ExchangeThreshold Management", function () {
    ...
    it("Should only allow REWARDS_MANAGER_ROLE to set exchange threshold", async function () {
      await expect(mockVault.connect(user).setExchangeThreshold(ethers.parseEther("2"))).to.be.revertedWithCustomError(
        mockVault,
        "AccessControlUnauthorizedAccount",
      );
    });
    ...
    it("Should revert if exchange threshold is zero", async function () {
      await expect(mockVault.connect(admin).setExchangeThreshold(0n)).to.be.revertedWithCustomError(
        mockVault,
  ...
  describe("claimRewards", function () {
    ...
    describe("Should only allow to compound when reaching the exchange threshold", async function () {
      it("Should revert if below threshold", async function () {
        ...
        await expect(
          mockVault.connect(user).compoundRewards(ethers.parseEther("1"), rewardTokens, await user.getAddress()),
        )
      ...
      it("Should not revert if above threshold", async function () {
        ...
        expect(
          await exchangeAsset.allowance(await user.getAddress(), await mockVault.getAddress()),
    ...
    it("Should revert on invalid reward token", async function () {
      ...
      await expect(mockVault.claimRewards([invalidToken], await user.getAddress())).to.be.revertedWith(
  ...
  describe("compoundRewards", function () {
    it("Compound with 10% treasury fee", async function () {
      ...
      // Perform compound
      await mockVault.connect(user).compoundRewards(amountToCompound, rewardTokenAddresses, await user.getAddress());
      ...
      const actualTreasuryFee1 =
        (await rewardToken1.balanceOf(await treasury.getAddress())) - initialTreasuryBalances[0];
      const actualTreasuryFee2 =
        (await rewardToken2.balanceOf(await treasury.getAddress())) - initialTreasuryBalances[1];
      ...
      expect(await mockVault.deposits(await exchangeAsset.getAddress())).to.equal(amountToCompound);
    });
    ...
    it("Should revert if amount is below threshold", async function () {
      ...
      await expect(
        mockVault
          .connect(user)
          .compoundRewards(belowThreshold, [await rewardToken1.getAddress()], await user.getAddress()),
      )
        .to.be.revertedWithCustomError(mockVault, "ExchangeAmountTooLow")
  ...
  describe("Access Control", function () {
    ...
    it("Should allow granting and revoking roles", async function () {
      ...
      await expect(mockVault.connect(user).setTreasury(await user.getAddress())).to.be.revertedWithCustomError(
        mockVault,
        "AccessControlUnauthorizedAccount",
      );
    });
