...
describe("DLoopDecreaseLeverageBase - MinLeftoverCollateralTokenAmount", function () {
  ...
  /**
   * Deploy fixture for DLoopDecreaseLeverageBase tests
   */
  async function deployFixture(): Promise<{
    ...
    decreaseLeverageMock = await DLoopDecreaseLeverageMockFactory.deploy(
      flashLender.target,
  ...
  beforeEach(async function () {
    ...
    decreaseLeverageMock = fixture.decreaseLeverageMock;
  ...
  describe("Adding Fresh Token (Baseline Behavior)", function () {
    it("should correctly add a new token with non-zero amount", async function () {
      ...
      await expect(decreaseLeverageMock.existingCollateralTokens(0)).to.be
        .reverted;
      ...
      await expect(
        ...
        .to.emit(decreaseLeverageMock, "MinLeftoverCollateralTokenAmountSet")
      ...
      await expect(decreaseLeverageMock.existingCollateralTokens(1)).to.be
        .reverted;
    });
  ...
  describe("Removing Existing Token (New Behavior)", function () {
    ...
    it("should handle removal from middle of array correctly", async function () {
      ...
      expect(await decreaseLeverageMock.existingCollateralTokens(3)).to.equal(
        tokenC.target,
      );
      ...
      const token0 = await decreaseLeverageMock.existingCollateralTokens(0);
      const token1 = await decreaseLeverageMock.existingCollateralTokens(1);
      const token2 = await decreaseLeverageMock.existingCollateralTokens(2);
      ...
      await expect(decreaseLeverageMock.existingCollateralTokens(3)).to.be
        .reverted;
  ...
  describe("Integration with Restricted Rescue Tokens", function () {
    it("should return updated array in getRestrictedRescueTokens after modifications", async function () {
      ...
      const initialTokens =
        await decreaseLeverageMock.getRestrictedRescueTokens();
  ...
  describe("Edge Cases", function () {
    it("should handle multiple removals from different positions", async function () {
      ...
      for (let i = 0; i < tokens.length; i++) {
        expect(await decreaseLeverageMock.existingCollateralTokens(i)).to.equal(
          tokens[i].target,
        );
      }
  ...
  describe("Gas Optimization", function () {
    it("should have reasonable gas costs for removal operations", async function () {
      ...
      const tx = await decreaseLeverageMock.setMinLeftoverCollateralTokenAmount(
        dLoopCore.target,
        tokenB.target,
        0,
      );
