...
describe("DLoopCoreLogic - Increase Leverage", () => {
  ...
  describe("getDebtBorrowAmountInBaseToIncreaseLeverage", () => {
    it("no subsidy", async () => {
      const { harness } = await deployHarness();
      const res = await harness.getDebtBorrowAmountInBaseToIncreaseLeveragePublic(1_000_000n, 0n);
      expect(res).to.equal(1_000_000n);
    });

    it("+100 bps", async () => {
      const { harness } = await deployHarness();
      const res = await harness.getDebtBorrowAmountInBaseToIncreaseLeveragePublic(1_000_000n, 10_000n);
      expect(res).to.equal((1_000_000n * (SCALE + 10_000n)) / SCALE);
    });

    it("+333 bps rounding down", async () => {
      const { harness } = await deployHarness();
      const res = await harness.getDebtBorrowAmountInBaseToIncreaseLeveragePublic(100n, 3_333n);
      expect(res).to.equal((100n * (SCALE + 3_333n)) / SCALE);
    });

    it("zero", async () => {
      const { harness } = await deployHarness();
      const res = await harness.getDebtBorrowAmountInBaseToIncreaseLeveragePublic(0n, 0n);
      expect(res).to.equal(0n);
    });

    it("tiny", async () => {
      const { harness } = await deployHarness();
      const res = await harness.getDebtBorrowAmountInBaseToIncreaseLeveragePublic(1n, 100n);
      expect(res).to.equal((1n * (SCALE + 100n)) / SCALE);
    });

    it("large x", async () => {
      const { harness } = await deployHarness();
      const res = await harness.getDebtBorrowAmountInBaseToIncreaseLeveragePublic(10n ** 30n, 0n);
      expect(res).to.equal(10n ** 30n);
    });

    it("large k", async () => {
      const { harness } = await deployHarness();
      const res = await harness.getDebtBorrowAmountInBaseToIncreaseLeveragePublic(10n ** 6n, 99_999n);
      expect(res).to.equal((10n ** 6n * (SCALE + 99_999n)) / SCALE);
    });

    it("mid", async () => {
      const { harness } = await deployHarness();
      const res = await harness.getDebtBorrowAmountInBaseToIncreaseLeveragePublic(123_456_789n, 25_000n);
      expect(res).to.equal((123_456_789n * (SCALE + 25_000n)) / SCALE);
    });

    it("k=1", async () => {
      const { harness } = await deployHarness();
      const res = await harness.getDebtBorrowAmountInBaseToIncreaseLeveragePublic(999_999n, 1n);
      expect(res).to.equal((999_999n * (SCALE + 1n)) / SCALE);
    });

    it("k=SCALE-1", async () => {
      const { harness } = await deployHarness();
      const res = await harness.getDebtBorrowAmountInBaseToIncreaseLeveragePublic(555_555n, SCALE - 1n);
  ...
  describe("getDebtBorrowTokenAmountToIncreaseLeverage", () => {
    ...
    it("3e18, +100bps", async () => {
      const { harness } = await deployHarness();
      const xBase = await harness.convertFromTokenAmountToBaseCurrencyPublic(3n * pow10(18n), 18, pow10(18n));
      const baseExpected = await harness.getDebtBorrowAmountInBaseToIncreaseLeveragePublic(xBase, 10_000n);
    ...
    it("debt cheaper", async () => {
      const { harness } = await deployHarness();
      const xBase = await harness.convertFromTokenAmountToBaseCurrencyPublic(pow10(18n), 18, pow10(18n));
      const baseExpected = await harness.getDebtBorrowAmountInBaseToIncreaseLeveragePublic(xBase, 25_000n);
      const tokenOut = await harness.getDebtBorrowTokenAmountToIncreaseLeveragePublic(
        pow10(18n),
        25_000n,
        18,
    ...
    it("edge k=SCALE-1", async () => {
      const { harness } = await deployHarness();
      const xBase = await harness.convertFromTokenAmountToBaseCurrencyPublic(pow10(18n), 18, pow10(18n));
      const baseExpected = await harness.getDebtBorrowAmountInBaseToIncreaseLeveragePublic(xBase, SCALE - 1n);
      const tokenOut = await harness.getDebtBorrowTokenAmountToIncreaseLeveragePublic(
        pow10(18n),
        SCALE - 1n,
        18,
