...
describe("DLoopCoreLogic - Decrease Leverage", () => {
  ...
  describe("getDebtRepayAmountInBaseToReachTargetLeverage", () => {
    it("table-driven property cases (new leverage >= target)", async () => {
      ...
      for (const tc of cases) {
        ...
        if (Lcur <= tc.TT) continue; // skip invalid case
  ...
  describe("getCollateralWithdrawAmountInBaseToDecreaseLeverage", () => {
    it("table-driven cases", async () => {
      ...
      for (const tc of cases) {
        ...
        expect(res >= tc.y).to.equal(true, tc.name);
      }
    });
  });
  ...
  describe("getCollateralWithdrawTokenAmountToDecreaseLeverage", () => {
    it("table-driven cases including error on zero input", async () => {
      ...
      for (const tc of success) {
        const yBase = await harness.convertFromTokenAmountToBaseCurrencyPublic(
          tc.y,
          Number(tc.dDec),
          tc.dPrice,
        );
        ...
        const tokenOut =
          await harness.getCollateralWithdrawTokenAmountToDecreaseLeveragePublic(
            tc.y,
            tc.k,
            Number(tc.cDec),
            tc.cPrice,
        ...
        expect(tokenOut).to.equal(expectedTokenOut, tc.name);
      }

      // error case: zero input debt
      await expect(
        harness.getCollateralWithdrawTokenAmountToDecreaseLeveragePublic(
          0,
          0,
          18,
          pow10(18n),
