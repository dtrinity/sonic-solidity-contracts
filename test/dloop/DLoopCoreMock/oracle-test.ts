...
describe.skip("DLoopCoreMock Oracle Tests", function () {
  ...
  describe("I. Basic Oracle Functionality", function () {
    it("Should set and get mock prices correctly", async function () {
      ...
      expect(await dloopMock.getMockPrice(await collateralToken.getAddress())).to.equal(collateralPrice);
      expect(await dloopMock.getMockPrice(await debtToken.getAddress())).to.equal(debtPrice);
    });
    ...
    it("Should handle different token decimals correctly", async function () {
      ...
      expect(await dloopMock.getMockPrice(await collateralToken.getAddress())).to.equal(price18Decimals);
      expect(await dloopMock.getMockPrice(await debtToken.getAddress())).to.equal(price18Decimals);
      expect(await dloopMock.getMockPrice(await otherToken.getAddress())).to.equal(price6Decimals);
    });
    ...
    it("Should update mock prices correctly", async function () {
      ...
      // Set initial price
      await dloopMock.setMockPrice(await collateralToken.getAddress(), initialPrice);
      expect(await dloopMock.getMockPrice(await collateralToken.getAddress())).to.equal(initialPrice);

      // Update price
      await dloopMock.setMockPrice(await collateralToken.getAddress(), updatedPrice);
      expect(await dloopMock.getMockPrice(await collateralToken.getAddress())).to.equal(updatedPrice);
    });
  });
  ...
  describe("II. Oracle Implementation Functions", function () {
    beforeEach(async function () {
      ...
      await dloopMock.setMockPrice(await collateralToken.getAddress(), ethers.parseEther("1500"));
    ...
    it("Should get asset price from oracle implementation", async function () {
      ...
      const price = await dloopMock.getAssetPriceFromOracle(await collateralToken.getAddress());
  ...
  describe("III. Price Conversion Functions", function () {
    beforeEach(async function () {
      ...
      await dloopMock.setMockPrice(await collateralToken.getAddress(), ethers.parseEther("1500"));
      await dloopMock.setMockPrice(await debtToken.getAddress(), ethers.parseEther("1"));
      await dloopMock.setMockPrice(await otherToken.getAddress(), ethers.parseEther("10"));
    });
    ...
    describe("Base Currency to Token Conversion", function () {
      it("Should convert from base currency to collateral token correctly", async function () {
        ...
        const tokenAmount = await dloopMock.convertFromBaseCurrencyToToken(amountInBase, await collateralToken.getAddress());
      ...
      it("Should convert from base currency to debt token correctly", async function () {
        ...
        const tokenAmount = await dloopMock.convertFromBaseCurrencyToToken(amountInBase, await debtToken.getAddress());
        expect(tokenAmount).to.equal(expectedTokenAmount);
      });
      ...
      it("Should convert from base currency to token with different decimals", async function () {
        ...
        const tokenAmount = await dloopMock.convertFromBaseCurrencyToToken(amountInBase, await otherToken.getAddress());
        expect(tokenAmount).to.equal(expectedTokenAmount);
      });

      it("Should handle zero amount conversion", async function () {
        const tokenAmount = await dloopMock.convertFromBaseCurrencyToToken(0, await collateralToken.getAddress());
    ...
    describe("Token to Base Currency Conversion", function () {
      it("Should convert from collateral token to base currency correctly", async function () {
        ...
        const baseAmount = await dloopMock.convertFromTokenAmountToBaseCurrency(tokenAmount, await collateralToken.getAddress());
      ...
      it("Should convert from debt token to base currency correctly", async function () {
        ...
        const baseAmount = await dloopMock.convertFromTokenAmountToBaseCurrency(tokenAmount, await debtToken.getAddress());
        expect(baseAmount).to.equal(expectedBaseAmount);
      });
      ...
      it("Should convert from token with different decimals to base currency", async function () {
        ...
        const baseAmount = await dloopMock.convertFromTokenAmountToBaseCurrency(tokenAmount, await otherToken.getAddress());
        expect(baseAmount).to.equal(expectedBaseAmount);
      });

      it("Should handle zero token amount conversion", async function () {
        const baseAmount = await dloopMock.convertFromTokenAmountToBaseCurrency(0, await collateralToken.getAddress());
    ...
    describe("Round-trip Conversion Tests", function () {
      it("Should maintain precision in round-trip conversions", async function () {
        ...
        const tokenAmount = await dloopMock.convertFromBaseCurrencyToToken(originalAmount, await collateralToken.getAddress());
        const backToBaseAmount = await dloopMock.convertFromTokenAmountToBaseCurrency(tokenAmount, await collateralToken.getAddress());
      ...
      it("Should handle precision for tokens with different decimals", async function () {
        ...
        const tokenAmount = await dloopMock.convertFromBaseCurrencyToToken(originalAmount, await otherToken.getAddress());
        const backToBaseAmount = await dloopMock.convertFromTokenAmountToBaseCurrency(tokenAmount, await otherToken.getAddress());
  ...
  describe("IV. Oracle Integration with Vault Operations", function () {
    ...
    it("Should use oracle prices for totalAssets calculation", async function () {
      ...
      const [collateralBase] = await dloopMock.getTotalCollateralAndDebtOfUserInBase(await dloopMock.getAddress());
      const expectedTotalAssets = await dloopMock.convertFromBaseCurrencyToToken(collateralBase, await collateralToken.getAddress());
    ...
    it("Should use oracle prices for deposit amount calculations", async function () {
      ...
      const collateralPrice = await dloopMock.getMockPrice(await collateralToken.getAddress());
      const debtPrice = await dloopMock.getMockPrice(await debtToken.getAddress());

      // Rough calculation check (accounting for leverage formula)
      const collateralValueInBase = (depositAmount * collateralPrice) / ethers.parseEther("1");
      const expectedDebtInBase = (collateralValueInBase * BigInt(200_0000)) / BigInt(300_0000); // 200% of collateral value for 3x leverage
      const expectedBorrowAmountCheck = (expectedDebtInBase * ethers.parseEther("1")) / debtPrice;

      expect(expectedBorrowAmount).to.be.closeTo(expectedBorrowAmountCheck, ethers.parseEther("1"));
    });
  });
  ...
  describe("V. Oracle Error Handling", function () {
    ...
    it("Should handle price updates during operations", async function () {
      ...
      // Set initial prices
      await dloopMock.setMockPrice(await collateralToken.getAddress(), ethers.parseEther("1200"));
      ...
      await dloopMock.connect(user).deposit(ethers.parseEther("25"), user.address);
    });
  });
  ...
  describe("VI. Oracle Price Edge Cases", function () {
    it("Should handle very large prices", async function () {
      ...
      await dloopMock.setMockPrice(await collateralToken.getAddress(), veryLargePrice);

      const retrievedPrice = await dloopMock.getMockPrice(await collateralToken.getAddress());
      expect(retrievedPrice).to.equal(veryLargePrice);

      // Should work in conversions
      const tokenAmount = await dloopMock.convertFromBaseCurrencyToToken(ethers.parseEther("1000000"), await collateralToken.getAddress());
    ...
    it("Should handle very small prices", async function () {
      ...
      await dloopMock.setMockPrice(await collateralToken.getAddress(), verySmallPrice);

      const retrievedPrice = await dloopMock.getMockPrice(await collateralToken.getAddress());
      expect(retrievedPrice).to.equal(verySmallPrice);

      // Should work in conversions
      const tokenAmount = await dloopMock.convertFromBaseCurrencyToToken(ethers.parseEther("1"), await collateralToken.getAddress());
    ...
    it("Should handle price precision correctly", async function () {
      ...
      await dloopMock.setMockPrice(await collateralToken.getAddress(), precisePrice);
      ...
      const tokenAmount = await dloopMock.convertFromBaseCurrencyToToken(baseAmount, await collateralToken.getAddress());
      const backToBase = await dloopMock.convertFromTokenAmountToBaseCurrency(tokenAmount, await collateralToken.getAddress());

      expect(backToBase).to.equal(baseAmount);
    });

    it("Should handle multiple price updates for same asset", async function () {
      const prices = [ethers.parseEther("1000"), ethers.parseEther("1500"), ethers.parseEther("800"), ethers.parseEther("2000")];
    ...
    it("Should handle simultaneous price updates for multiple assets", async function () {
      ...
      expect(await dloopMock.getMockPrice(await collateralToken.getAddress())).to.equal(collateralPrice);
      expect(await dloopMock.getMockPrice(await debtToken.getAddress())).to.equal(debtPrice);
      expect(await dloopMock.getMockPrice(await otherToken.getAddress())).to.equal(otherPrice);

      // Verify oracle function works for all
      expect(await dloopMock.getAssetPriceFromOracle(await collateralToken.getAddress())).to.equal(collateralPrice);
      expect(await dloopMock.getAssetPriceFromOracle(await debtToken.getAddress())).to.equal(debtPrice);
      expect(await dloopMock.getAssetPriceFromOracle(await otherToken.getAddress())).to.equal(otherPrice);
    });
  });
});
