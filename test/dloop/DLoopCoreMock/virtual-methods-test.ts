...
describe("DLoopCoreMock Virtual Methods Tests", function () {
  ...
  describe("IV. Integration Tests", function () {
    it("Should handle complete supply and borrow flow", async function () {
      ...
      // Check final state
      expect(await dloopMock.getMockCollateral(user1, await collateralToken.getAddress())).to.equal(supplyAmount);
      expect(await dloopMock.getMockDebt(user1, await debtToken.getAddress())).to.equal(borrowAmount);
    ...
    it("Should handle complete repay and withdraw flow", async function () {
      ...
      // Repay part of debt
      await dloopMock.testRepayDebtToPoolImplementation(await debtToken.getAddress(), repayAmount, user1);
