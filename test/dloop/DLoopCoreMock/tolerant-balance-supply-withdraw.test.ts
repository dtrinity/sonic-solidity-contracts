...
describe("DLoopCoreMock – tolerant balance differences (supply / withdraw)", function () {
  ...
  describe("_supplyToPool wrapper", function () {
    it("allows 1 wei rounding difference on supply", async function () {
      ...
      await expect(dloopMock.testSupplyToPool(await collateralToken.getAddress(), 100n, vault)).to.not.be.reverted;
      ...
      await expect(dloopMock.testSupplyToPool(await collateralToken.getAddress(), 100n, vault)).to.be.reverted;
    });
  });
  ...
  describe("_withdrawFromPool wrapper", function () {
    it("allows 1 wei rounding difference on withdraw", async function () {
      ...
      await dloopMock.testSupplyToPoolImplementation(await collateralToken.getAddress(), 100n, vault);
      await dloopMock.testSupplyToPoolImplementation(await collateralToken.getAddress(), 100n, vault);
      ...
      await dloopMock.setTransferPortionBps(99 * ONE_PERCENT_BPS); // 99.00%
      await expect(dloopMock.testWithdrawFromPool(await collateralToken.getAddress(), 100n, vault)).to.not.be.reverted;
      ...
      await dloopMock.setTransferPortionBps(98 * ONE_PERCENT_BPS); // 98.00%
      await expect(dloopMock.testWithdrawFromPool(await collateralToken.getAddress(), 100n, vault)).to.be.reverted;
    });
  });
});
