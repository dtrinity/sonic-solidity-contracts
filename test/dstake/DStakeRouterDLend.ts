  ...
  describe(`DStakeRouterDLend for ${config.DStakeTokenSymbol}`, function () {
    ...
    describe("Initialization and State", function () {
      it("should set correct immutable addresses", async function () {
        expect(await router.dStakeToken()).to.equal(await DStakeToken.getAddress());
        expect(await router.collateralVault()).to.equal(await collateralVault.getAddress());
      ...
      it("should grant DSTAKE_TOKEN_ROLE to the DStakeToken address", async function () {
        ...
        expect(await router.hasRole(tokenRole, await DStakeToken.getAddress())).to.be.true;
      });

      it("defaultDepositVaultAsset should be zero address initially", async function () {
        expect(await router.defaultDepositVaultAsset()).to.equal(vaultAssetAddress);
      });

      it("vaultAssetToAdapter mapping should be empty initially", async function () {
        expect(await router.vaultAssetToAdapter(vaultAssetAddress)).to.equal(adapterAddress);
      });
    });

    describe("Role-Based Access Control & Configuration", function () {
      it("admin can add a new adapter", async function () {
        await expect(router.connect(user1Signer).addAdapter(vaultAssetAddress, adapterAddress))
      ...
      it("non-admin cannot add adapter", async function () {
        await expect(router.connect(user2Signer).addAdapter(vaultAssetAddress, adapterAddress)).to.be.reverted;
      });

      it("cannot add adapter with zero addresses", async function () {
        await expect(router.connect(user1Signer).addAdapter(ZeroAddress, adapterAddress)).to.be.revertedWithCustomError(
          router,
          "ZeroAddress",
        );
      ...
      it("admin can remove an adapter", async function () {
        // First add
        await router.connect(user1Signer).addAdapter(vaultAssetAddress, adapterAddress);
        // Then remove
        await expect(router.connect(user1Signer).removeAdapter(vaultAssetAddress))
          .to.emit(router, "AdapterRemoved")
          .withArgs(vaultAssetAddress, adapterAddress);
        expect(await router.vaultAssetToAdapter(vaultAssetAddress)).to.equal(ZeroAddress);
      });

      it("non-admin cannot remove adapter", async function () {
        await expect(router.connect(user2Signer).removeAdapter(vaultAssetAddress)).to.be.reverted;
      });

      it("admin can set defaultDepositVaultAsset", async function () {
        // Add adapter first
        await router.connect(user1Signer).addAdapter(vaultAssetAddress, adapterAddress);
        await expect(router.connect(user1Signer).setDefaultDepositVaultAsset(vaultAssetAddress))
      ...
      it("non-admin cannot set defaultDepositVaultAsset", async function () {
        await expect(router.connect(user2Signer).setDefaultDepositVaultAsset(vaultAssetAddress)).to.be.reverted;
      });
    ...
    describe("Deposit and Withdraw", function () {
      ...
      beforeEach(async function () {
        ...
        await ethers.provider.send("hardhat_impersonateAccount", [DStakeTokenAddress]);
        // Fund impersonated DStakeToken with ETH for gas
        await ethers.provider.send("hardhat_setBalance", [DStakeTokenAddress, "0x1000000000000000000"]);
      });

      it("non-DStakeToken cannot call deposit", async function () {
        await expect(router.connect(user1Signer).deposit(depositAmount)).to.be.reverted;
      });
      ...
      it("DStakeToken can deposit, emits event and deposits vault asset to collateralVault", async function () {
        ...
        const [, previewShares] = await adapter.previewConvertToVaultAsset(depositAmount);

        const beforeBal = await vaultAssetToken.balanceOf(collateralVaultAddress);

        // Check event emission and balances
        await expect(router.connect(DStakeTokenSigner).deposit(depositAmount)).to.emit(router, "RouterDeposit");

        const afterBal = await vaultAssetToken.balanceOf(collateralVaultAddress);
      ...
      it("reverts when adapter under-delivers vault shares", async function () {
        ...
        const MintableFactory = await ethers.getContractFactory("TestMintableERC20", deployerSigner);
        const mockVaultAsset = await MintableFactory.deploy("Mock Vault", "MVLT", 18);
        await mockVaultAsset.waitForDeployment();

        // Deploy under-delivering adapter (90% factor)
        const UDAdapterFactory = await ethers.getContractFactory("MockUnderDeliveringAdapter", deployerSigner);
        ...
        await router.connect(user1Signer).addAdapter(await mockVaultAsset.getAddress(), await udAdapter.getAddress());
        await router.connect(user1Signer).setDefaultDepositVaultAsset(await mockVaultAsset.getAddress());
      ...
      it("non-DStakeToken cannot call withdraw", async function () {
        await expect(router.connect(user1Signer).withdraw(depositAmount, user1Addr, user1Addr)).to.be.reverted;
      });
      ...
      it("captures surplus on positive slippage (bonus redeem adapter)", async function () {
        ...
        const AdapterFactory = await ethers.getContractFactory("MockAdapterPositiveSlippage", deployerSigner);
        const bonusAdapter = await AdapterFactory.deploy(await dStableToken.getAddress(), collateralVaultAddress);
        ...
        await router.connect(user1Signer).addAdapter(bonusVaultAsset, bonusAdapterAddr);
        await router.connect(user1Signer).setDefaultDepositVaultAsset(bonusVaultAsset);
        ...
        // user1 deposit
        await dStableToken.connect(user1Signer).approve(await DStakeToken.getAddress(), depositAmt);
        ...
        const vaultToken = await ethers.getContractAt("MockERC4626Simple", bonusVaultAsset);
        ...
        await DStakeToken.connect(user1Signer).withdraw(withdrawAmt, user1Addr, user1Addr);
    ...
    describe("Exchange Assets Using Adapters", function () {
      ...
      beforeEach(async function () {
        ...
        await dStableToken.connect(DStakeTokenSigner).approve(routerAddress, depositAmount);
        await router.connect(DStakeTokenSigner).deposit(depositAmount);
        await router.connect(user1Signer).grantRole(await router.COLLATERAL_EXCHANGER_ROLE(), user1Addr);
      });
    ...
    describe("Exchange Assets", function () {
      ...
      beforeEach(async function () {
        ...
        await ethers.provider.send("hardhat_impersonateAccount", [collateralVaultAddress]);
        // Fund impersonated collateralVault with ETH for gas
        await ethers.provider.send("hardhat_setBalance", [collateralVaultAddress, "0x1000000000000000000"]);
        const vaultSigner = await ethers.getSigner(collateralVaultAddress);
        await vaultAssetToken.connect(vaultSigner).transfer(user1Addr, exchangeAmount);
        await ethers.provider.send("hardhat_stopImpersonatingAccount", [collateralVaultAddress]);
        // Approve router to spend solver's vaultAsset for exchange
        await vaultAssetToken.connect(user1Signer).approve(routerAddress, exchangeAmount);
      });
      ...
      it("reverts if adapter not found", async function () {
        await expect(
          router.connect(user1Signer).exchangeAssets(ZeroAddress, vaultAssetAddress, exchangeAmount, 0),
        ).to.be.revertedWithCustomError(router, "ZeroAddress");
        await expect(
          router.connect(user1Signer).exchangeAssets(vaultAssetAddress, ZeroAddress, exchangeAmount, 0),
        ).to.be.revertedWithCustomError(router, "ZeroAddress");
      });
      ...
      it("can exchange assets and emits event and net solver balances unchanged", async function () {
        const dStableValueIn = await adapter.previewConvertFromVaultAsset(exchangeAmount);
        const [, expectedToVault] = await adapter.previewConvertToVaultAsset(dStableValueIn);
