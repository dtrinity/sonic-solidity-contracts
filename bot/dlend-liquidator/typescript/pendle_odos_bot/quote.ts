...
export async function getPTOdosSwapQuote(
  ...
  if (underlyingAsset.toLowerCase() !== borrowTokenAddress.toLowerCase()) {
    ...
    console.log("Odos quote request:", odosQuoteRequest);

    const odosQuote = await odosClient.getQuote(odosQuoteRequest);
    console.log("Odos quote response:", {
      outAmounts: odosQuote.outAmounts,
  ...
  console.log("Created PT swap data:", {
    underlyingAsset: ptSwapData.underlyingAsset,
    expectedUnderlying: ptSwapData.expectedUnderlying,
    pendleTarget: ptSwapData.pendleTarget,
    odosTarget: ptSwapData.odosTarget,
    pendleCalldataLength: ptSwapData.pendleCalldata.length,
    odosCalldataLength: ptSwapData.odosCalldata.length,
  });
