...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  console.log(
    "🤖 Deploying DLoop Reward Compounder Periphery on Sonic Mainnet...",
  );
  ...
  if (
    ...
    config.collateralAddress === ethers.ZeroAddress
  ) {
    throw new Error("COLLATERAL address is not configured");
  }

  if (
    !config.flashLenderAddress ||
    config.flashLenderAddress === ethers.ZeroAddress
  ) {
    throw new Error("FLASH_LENDER address is not configured");
  }

  if (
    !config.odosRouterAddress ||
    config.odosRouterAddress === ethers.ZeroAddress
  ) {
  ...
  console.log(
    `✅ Deployed RewardCompounderDLendOdos at ${deploymentResult.address}`,
  );
  ...
  try {
    ...
    console.log(
      `   COLLATERAL matches: ${collateralAddress === config.collateralAddress}`,
    );
...
func.tags = ["reward-compounder", "periphery"];
func.dependencies = [];
